...
function head(meta,siteUrl,preview=false) {
  ...
  if(meta.indexable){
    ...
    // Extra structured data travels inside the single ld+json block: a page may
    // declare one primary type, and the release gate enforces exactly one block.
...
function apply(html,meta,siteUrl,preview=false) {
  ...
  html=html.replace(/<head>([\s\S]*?)<\/head>/i,(_,contents)=>'<head>'+contents.replace(/<title\b[^>]*>[\s\S]*?<\/title>/gi,'').replace(/<meta\b[^>]*(?:name="(?:description|robots|twitter:[^"]*)"|property="og:[^"]*")[^>]*>/gi,'').replace(/<link\b[^>]*rel="(?:canonical|alternate|icon|apple-touch-icon)"[^>]*>/gi,'').replace(/<script\b[^>]*type="application\/ld\+json"[^>]*>[\s\S]*?<\/script>/gi,'')+head(meta,siteUrl,preview)+'</head>');
...
module.exports={metadata,head,icons,apply,breadcrumbHtml,sitemap,robots,fingerprint};
