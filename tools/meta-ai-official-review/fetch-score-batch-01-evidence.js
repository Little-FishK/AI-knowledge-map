/* Freeze first-party Meta evidence for the first 60-record value-score batch. */
"use strict";
...
const outputPath = path.join(PROJECT_ROOT, "proposals", "official-technical", `meta-ai-value-score-batch-${suffix}-evidence.json`);
...
async function fetchOne(record, sequence) {
  const response = await fetch(record.canonicalUrl, { headers:{ "user-agent":"ai-knowledge-map-meta-review/1.0" } });
  ...
  const pageTitle = plainText(html.match(/<h1\b[^>]*>([\s\S]*?)<\/h1>/i)?.[1])
    || plainText(html.match(/<title>([\s\S]*?)<\/title>/i)?.[1]).replace(/\s*[-|]\s*Meta Model API\s*$/i, "");
  return {
    sequence,
    ...record,
    pageTitle:pageTitle || record.title,
    pageDescription:meta(html, "description") || meta(html, "og:description") || record.description,
    contentVerification:{
      ...
      articleTextChars:plainText(article).length,
...
async function main() {
  if (fs.existsSync(outputPath)) {
    ...
    process.stdout.write(`${frozen.records.length} frozen Meta evidence records already exist for batch ${batchNumber}\n`);
    return;
  }
  const inventory = JSON.parse(fs.readFileSync(inventoryPath, "utf8"));
  const expectedCount = batchNumber === 7 ? 37 : 60;
  const candidates = inventory.records.filter(record => record.reviewStatus === "pending-importance-review").slice(0, expectedCount);
  if (candidates.length !== expectedCount) throw new Error(`Expected ${expectedCount} Meta candidates, got ${candidates.length}`);
  let cursor = 0;
  ...
  process.stdout.write(`${records.length} Meta evidence records written for batch ${batchNumber}\n`);
}
