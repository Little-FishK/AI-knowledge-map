...
function runtimeDeepDiveSource(nodeId, page) {
  return `/* Generated by tools/build-deepdive-runtime.js. Do not edit. */\n`
    + "window.DEEPDIVE=window.DEEPDIVE||{};"
    + `window.DEEPDIVE[${JSON.stringify(nodeId)}]=${JSON.stringify(page)};\n`;
}
...
function loadRuntimeManifest(root) {
  const file = withinRoot(root, "data/deepdive-runtime/manifest.js");
  const context = { window: {} };
  vm.createContext(context);
  vm.runInContext(readTextIfExists(file), context, { filename: file });
  return context.window.DEEPDIVE_RUNTIME || { ids: [] };
}
...
function buildNodeApplyPlan(packageDir, options = {}) {
  ...
  if (!blockers.length) {
    ...
    transformIndex(indexContent, manifest.node.id);
    ...
    targets = [
      ...
      targetRecord(
        ...
        runtimeDeepDiveSource(manifest.node.id, finalDeepDive)
      ),
      targetRecord(
        root,
        "data/deepdive-runtime/manifest.js",
        runtimeManifestSource([...(runtimeManifest.ids || []), manifest.node.id])
      )
    ];
    operations = [
      ...
      { type: "register-lazy-deepdive", nodeId: manifest.node.id }
    ];
  }
  const plan = {
    schemaVersion: 1,
