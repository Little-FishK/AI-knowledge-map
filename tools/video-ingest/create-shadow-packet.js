  ...
  return {
    ...
    task: {
      ...
      rules: [
        ...
        "nearestNodes 必须是现有 node id 字符串数组，例如 [\"speech\", \"privacy\"]，不得输出带 nodeId/reason 的对象。",
        "claims 每项必须使用 { text, sourceUrls, evidenceRefs }；sourceUrls 只能引用 externalSources 中登记的 URL，evidenceRefs 使用 { start, end }。",
        "proposedLearningPath 必须使用 { phase, order, afterNodes, beforeNodes, rationale }；afterNodes 与 beforeNodes 都是现有 node id 字符串数组。",
