...
const {
  ...
  validateProposal,
  sha256
} = require("../../tools/video-ingest/core");
const { buildContext } = require("../../tools/video-ingest/build-context");
const { createDraft } = require("../../tools/video-ingest/create-proposal");
const { finalizeProposal } = require("../../tools/video-ingest/finalize-proposal");

["evidence.schema.json", "proposal.schema.json", "approval.schema.json"].forEach(file => {
  const schema = JSON.parse(fs.readFileSync(path.join(PROJECT_ROOT, "tools", "video-ingest", "schemas", file), "utf8"));
...
try {
  ...
  const create = spawnSync(process.execPath, [
    path.join(PROJECT_ROOT, "tools", "video-ingest", "create-proposal.js"),
  ...
  const validate = spawnSync(process.execPath, [
    path.join(PROJECT_ROOT, "tools", "video-ingest", "validate-proposal.js"),
    "--evidence", evidenceFile,
