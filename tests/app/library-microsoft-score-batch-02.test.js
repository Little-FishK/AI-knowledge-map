...
const score = JSON.parse(fs.readFileSync(path.join(root, "proposals", "official-technical", "microsoft-value-score-batch-02.json"), "utf8"));
...
assert.deepStrictEqual(score.summary, { reviewed:60, retained:47, removed:13 });
const batchInventoryRecords = inventory.records.filter(record => record.scoreBatch === "microsoft-value-score-batch-02");
assert.strictEqual(batchInventoryRecords.length, 60);
assert.strictEqual(batchInventoryRecords.filter(record => String(record.status).startsWith("admitted-")).length, 47);
assert.strictEqual(batchInventoryRecords.filter(record => record.status === "rejected-low-value-score").length, 13);
...
for (const relative of ["data/library.js", "data/library-official-technical.js", "data/library-official-microsoft-importance-01.js", "data/library-official-microsoft-importance-02.js"]) {
