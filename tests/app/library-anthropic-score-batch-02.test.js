...
const batches = [1, 2].map(number => require(path.join(PROJECT_ROOT, "proposals", "official-technical", `anthropic-value-score-batch-0${number}.json`)));
...
assert.strictEqual(new Set(results.map(result => result.id)).size, 120);
assert.strictEqual(batches[1].summary.retained, 51);
assert.strictEqual(batches[1].summary.removed, 9);
assert(items.length <= 290, "后续批次只能继续收紧 Anthropic 前台数量");
...
console.log("✓ Anthropic 第二批 60 份评分：保留 51，淘汰 9；批次结论在后续收紧后仍成立");
