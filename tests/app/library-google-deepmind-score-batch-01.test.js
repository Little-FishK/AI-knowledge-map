...
const score = JSON.parse(fs.readFileSync(path.join(
  root, "proposals", "official-technical", "google-deepmind-value-score-batch-01.json"
), "utf8"));
...
assert.strictEqual(items.filter(item => item.scoreBatch === "google-deepmind-value-score-batch-01").length, 50);
console.log("✓ Google/DeepMind 第一批 60 份价值评分通过（保留 50，移除 10）");
