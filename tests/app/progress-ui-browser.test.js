...
(async()=>{
  ...
  try {
    ...
    // Exercise progress UI with synthetic reading content, never a Stage 2 page body.
    await context.route('**/data/deepdive-runtime/manifest.js',route=>route.fulfill({contentType:'text/javascript',body:'window.DEEPDIVE_RUNTIME={base:"data/deepdive-runtime",ids:["supervised-learning"],revision:"progress-fixture"};'}));
    await context.route('**/data/deepdive-runtime/supervised-learning.js',route=>route.fulfill({contentType:'text/javascript',body:'window.DEEPDIVE=window.DEEPDIVE||{};window.DEEPDIVE["supervised-learning"]={title:"Progress fixture",html:"<p>Synthetic reading page.</p>"};'}));
    ...
    await card.waitFor();
    const account=page.locator('#settings-account [data-progress-account-button]');
    await account.waitFor({state:'attached'});assert.equal(await account.textContent(),'账户');
    ...
    assert.equal(await account.textContent(),'账户');
    ...
    await account.click();assert.equal(await page.locator('[data-email-form]').count(),0,'progress failure must not undo authentication');
    await page.locator('.progress-account-close').click();
    await page.locator('#settings-close').click();
    await page.evaluate(()=>{location.hash='#/concept/supervised-learning';});
    await card.waitFor();
