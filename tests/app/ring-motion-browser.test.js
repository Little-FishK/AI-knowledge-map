...
(async()=>{
  ...
  try {
    for (const reduced of [false,true]) {
      ...
      const dragStart=await page.evaluate(()=>{const r=__cy.container().getBoundingClientRect();return{x:r.right-90,y:r.bottom-45};});
      ...
      assert.deepEqual(await page.evaluate(()=>({...__cy.pan()})),initialPan,'background press starts fading before movement');
      await page.waitForTimeout(200);
      assert.equal(await page.evaluate(()=>__cy.edges('.viewport-drag-hidden').length),0,'fade is still in progress after 200ms');
