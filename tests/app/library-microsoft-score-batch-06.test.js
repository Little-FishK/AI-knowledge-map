...
const score = JSON.parse(fs.readFileSync(path.join(root, "proposals", "official-technical", "microsoft-value-score-batch-06.json"), "utf8"));
...
const batchInventoryRecords = inventory.records.filter(record => record.scoreBatch === "microsoft-value-score-batch-06");
assert.strictEqual(batchInventoryRecords.length, 60);
assert.strictEqual(batchInventoryRecords.filter(record => String(record.status).startsWith("admitted-")).length, 39);
assert.strictEqual(batchInventoryRecords.filter(record => record.status === "rejected-low-value-score").length, 21);
