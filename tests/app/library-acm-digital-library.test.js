...
const expectedIds = [
  "acm-3726302-3729904", "acm-3715275-3732137", "acm-3715275-3732170",
  "acm-3715275-3732202", "acm-3731148", "acm-3730841", "acm-3730843",
  "acm-3731175", "acm-3730937"
];
const records = window.PRO_LIBRARY.items.filter(item => item.sourceSubcategory === "acm-dl");
assert.deepEqual(records.map(item => item.id).sort(), expectedIds.sort());
for (const record of records) {
  assert.equal(record.discoveryOnly, true);
  assert.equal(record.importanceReview.status, "needs-evidence");
  assert.match(record.reviewStatus, /没有 AI 重大贡献外部说明/);
