...
const batches = [1, 2, 3, 4].map(number => require(path.join(PROJECT_ROOT, "proposals", "official-technical", `anthropic-value-score-batch-0${number}.json`)));
...
assert.strictEqual(new Set(results.map(result => result.id)).size, 240);
assert.strictEqual(batches[3].summary.retained, 49);
assert.strictEqual(batches[3].summary.removed, 11);
assert(items.length <= 273, "后续批次只能继续收紧 Anthropic 前台数量");
...
console.log("✓ Anthropic 第四批 60 份评分：保留 49，淘汰 11；批次结论在后续收紧后仍成立");
