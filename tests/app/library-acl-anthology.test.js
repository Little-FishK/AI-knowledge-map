...
const records = window.PRO_LIBRARY.items.filter(item => item.sourceSubcategory === "acl-anthology");
assert.equal(records.length, 52);
assert.equal(records.filter(item => item.importanceReview.status === "needs-evidence").length, 51);
assert.equal(records.filter(item => item.importanceReview.status === "recheck").length, 1);
assert.ok(records.every(item => item.discoveryOnly === true));
assert.ok(records.every(item => /上线不代表通过/.test(item.reviewStatus)));
