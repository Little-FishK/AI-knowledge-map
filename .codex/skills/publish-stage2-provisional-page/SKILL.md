...
description: >-
  ...
  responses with a fresh content-generation agent, import the hash-bound
  candidate, audit it, allow at most one repair and scoped verification, and
  either publish a red provisional review page or hold at manual review. Use for
  one-node understanding-page preparation, safe resumption, or scheduled Stage
  2 runs. Never use it to grant final human approval.
---

# Publish a Stage 2 Provisional Page

Use the `ai-knowledge-map-stage2` MCP server as the only Stage 2 interface. Process exactly one existing official-recommendation node per invocation, resume safely from its controller state, and stop after the requested manual-review action. Never represent provisional publication as L3 Pass or final approval.
...
If either value is missing, ask for it. Do not choose a page or publication action implicitly. Treat the user's order label as an official recommendation order, not as a page ID; resolve the page ID through the controller.
...
4. Spawn fresh role-locked audit or repair agents only as directed by controller state. Audit cannot edit content. Repair receives only sanitized defects. Permit at most one repair, followed by verification limited to the first audit's findings.
...
- Switching nodes always requires a new agent with no inherited context.
...
- After an accepted submission, follow the controller's `uiCleanup` instruction. Archive only the completed Codex task; archiving is not a Stage 2 state change.

For reusable scheduled-task text, read [references/scheduled-prompts.md](references/scheduled-prompts.md).
