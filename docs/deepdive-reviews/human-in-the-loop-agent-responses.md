# 人在回路：把稀缺的人类判断放到能改变风险的节点 Agent Responses

> 页面：human-in-the-loop。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
