# 音频生成：在波形、频谱与离散声码之间建模时间结构 Agent Responses

> 页面：audio-generation。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 波形是高频连续信号，直接逐样本生成代价巨大
...
## 5. 原创图：文本与旋律条件共同引导声码生成，再由codec还原波形
