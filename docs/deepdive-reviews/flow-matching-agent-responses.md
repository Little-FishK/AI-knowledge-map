# 流匹配：直接学习把噪声连续运输到数据的速度场 Agent Responses

> 页面：flow-matching。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 生成可以看成把一团简单噪声搬成数据分布
...
## 8. 条件生成与classifier-free guidance会改变原速度场
