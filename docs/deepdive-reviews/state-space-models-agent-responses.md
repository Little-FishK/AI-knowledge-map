# 状态空间模型 SSM：用可控动态系统压缩长序列 Agent Responses

> 页面：state-space-models。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
...
## 3. 同一线性 SSM 也可以看成一维卷积

线性状态空间模型既可以按时间递推，也可以把整段输入写成一维卷积。递推形式逐步更新状态，天然适合流式处理，但后一步依赖前一步，直接按时间展开时不容易并行。把递推式反复代入，就能消去中间状态，得到第 k 步输出：
