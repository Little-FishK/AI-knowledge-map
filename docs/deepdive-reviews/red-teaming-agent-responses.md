# AI 红队测试：用威胁模型系统寻找可造成真实影响的失败链 Agent Responses

> 页面：red-teaming。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 先定义资产、攻击者能力和不可接受影响
