# AI 治理：把风险判断变成有人负责、可留证、可停止的制度 Agent Responses

> 页面：governance。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 治理对象是社会技术系统，不只是模型文件
