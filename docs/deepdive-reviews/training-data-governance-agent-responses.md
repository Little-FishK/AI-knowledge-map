# 训练数据治理：让模型所学内容可追溯、可审计、可处置 Agent Responses

> 页面：training-data-governance。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
