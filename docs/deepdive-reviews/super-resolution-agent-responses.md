# 超分辨率：从退化观测反推高分辨率，不等于找回真实细节 Agent Responses

> 页面：super-resolution。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
