# AI 隐私：沿数据流控制可识别性、记忆与越权使用 Agent Responses

> 页面：privacy。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 先问数据能否关联到人，而不只找姓名和手机号
