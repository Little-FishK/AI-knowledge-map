# 智能体框架：抽象运行循环，但不外包正确性与控制权 Agent Responses

> 页面：agent-frameworks。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
