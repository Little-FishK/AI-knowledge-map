# 系统提示：在概率模型里声明行为契约，而不是建立安全边界 Agent Responses

> 页面：system-prompt。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 系统提示仍然只是模型看到的一串 token
...
| 来源 | 信任角色 | 应当怎样处理 |
| --- | --- | --- |
| 系统/开发者 | 应用策略与任务契约 | 约束低层指令 |
| 用户 | 目标和输入 | 在高层允许范围内执行 |
| 检索/网页/工具输出 | 不可信数据 | 作为事实候选，不自动当命令 |
| 历史摘要/记忆 | 派生状态 | 带来源、作用域和可纠正性 |
