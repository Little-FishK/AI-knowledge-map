# 生成对抗网络 GAN：让生成器在动态判别对手中学习数据分布 Agent Responses

> 页面：gan。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 生成器没有正确答案标签，只能从判别器获得方向
...
## 9. Wasserstein目标用critic估计更连续的分布距离
