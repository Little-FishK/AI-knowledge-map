# MCP 架构：让 Host 在隔离边界内连接外部能力 Agent Responses

> 页面：mcp-architecture。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 协议解决的是复用，不是模型是否会调用工具
...
## 5. 原创图：Host 是跨 Server 的唯一协调与信任汇合点
...
## 11. 先澄清：协议、SDK、Server 与 Agent 不是同一层
