# 护栏 Guardrails：用纵深控制限制概率系统的损失半径 Agent Responses

> 页面：guardrails。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
...
旧阈值：5 × 1000 + 18 × 5 = 5090 元
新阈值：1 × 1000 + 135 × 5 = 1675 元
