# AI 内容检测与溯源：区分统计猜测、水印信号和签名来源 Agent Responses

> 页面：content-detection。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 先定义要回答的命题，而不是泛问“真假”
