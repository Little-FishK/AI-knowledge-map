# LLM 应用评测：测量整条系统是否完成真实任务 Agent Responses

> 页面：evaluation。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。
...
新版本的总分从 80.0 升到 83.2，但越权请求的通过率也明显下滑，应该上线吗？把这个问题算清楚，就能看清平均提升和发布决策之间的真实关系。
