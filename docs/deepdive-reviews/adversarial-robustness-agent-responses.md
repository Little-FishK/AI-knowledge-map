# 对抗鲁棒性：在明确扰动集合内寻找最坏情况，而非宣称模型“抗攻击” Agent Responses

> 页面：adversarial-robustness。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 自然准确率只测数据分布附近，不测主动最坏情况
