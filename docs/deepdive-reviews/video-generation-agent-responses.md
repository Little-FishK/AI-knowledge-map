# 视频生成：联合建模空间外观、时间运动与跨镜头状态 Agent Responses

> 页面：video-generation。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 视频多一个时间轴，计算和一致性问题一起放大
