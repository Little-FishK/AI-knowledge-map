# 数据投毒：攻击者怎样用少量训练信号改变模型的特定行为 Agent Responses

> 页面：data-poisoning。以下内容按原页章节顺序，由同一个 content-generation Agent 对每章执行控制器返回的完整固定提示词后原样汇总。常见误解与自测章节未发送。

## 1. 先区分恶意训练影响与普通数据质量问题
