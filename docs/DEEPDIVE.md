...
- 另有“上下文学习”和“监督学习”2 个非核心扩展页，因此当前深读页总数为 **30**。
- 当前没有核心节点缺页、深读页指向不存在节点或脚本加载失败的情况。
- 同口径质量复核结果为 **28/28 均不低于 88 分**，最低 88、平均 91.4；详见 [DEEPDIVE_QUALITY_AUDIT.md](DEEPDIVE_QUALITY_AUDIT.md)。
...
- 人工复核按 100 分制：准确性 25、认知连续与依赖 20、原理深度 20、示例与消歧 15、总结与练习 10、来源与可维护性 10。
- 核心页发布线为 **88 分**；快速变化内容必须写清限定条件，并记录资料访问日期。
- 每次修改深读页后运行：`node tools/validate-deepdives.js`。脚本按 `graph.core` 动态检查全部核心页覆盖、学习目标、因果链、自测与答案、至少 2 个资料链接及访问日期；它不能替代事实与教学质量的人工审校。
