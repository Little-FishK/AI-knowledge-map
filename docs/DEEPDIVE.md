...
- `data/graph.js` 的核心列表共有 **23 个节点**。
- 23 个核心节点均已注册对应深读页，覆盖率 **23/23（100%）**。
- 另有“上下文学习”和“监督学习”2 个非核心扩展页，因此当前深读页总数为 **25**。
- 当前没有核心节点缺页、深读页指向不存在节点或脚本加载失败的情况。
- 同口径质量复核结果为 **23/23 均不低于 88 分**，最低 88、平均 91.4；详见 [DEEPDIVE_QUALITY_AUDIT.md](DEEPDIVE_QUALITY_AUDIT.md)。

核心覆盖清单：LLM、Transformer、注意力机制、嵌入、上下文窗口、神经网络、预训练、微调、多模态、RAG、检索、提示工程、思维链、AI Agent、工具调用、MCP、代码生成、扩散模型、图像生成、幻觉、提示注入、AI 对齐、推理模型。
...
- 每次修改深读页后运行：`node tools/validate-deepdives.js`。脚本检查 23 个核心页覆盖、学习目标、因果链、自测与答案、至少 2 个资料链接及访问日期；它不能替代事实与教学质量的人工审校。
