...
每个 `core` 章节必须有一份独立六问审计，正式位置为
`docs/deepdive-audits/<page-id>.json` 的 `sections[]`。迁移期间仍可读取
`quality.sectionContracts[]`，但会产生 `audit.inline-section-contracts`
编辑/迁移提示，不能把正文作者自报的合同当作独立审计完成。
...
每项同时包含不少于 16 字的 `answer` 和当前章节真实存在的 `evidence`。六项证据不得复用同一个占位片段；证据不能从别的章节借用。缺少章节合同产生 `section.missing-contract`，缺少任一问题产生 `section.missing-*`，复用证据产生 `section.reused-evidence`，均为不可补偿阻断。

外置审计必须包含当前正文的 `pageHash`。正文发生任何变化后，旧审计产生
`audit.page-hash-mismatch` 并立即失效。正文写作与审计必须分开进行：负责生成
或改写正文的模型不得同步填写审计，更不得为满足审计而在节尾补六个答案。

存在证据片段不等于回答充分。审计器必须确认 `evidence` 能定位到真实正文块，并回到该核心章节全文，按六个问题分别检查其最低语义结构：

- `definition` 必须真正界定、描述或说明对象是什么，不能只写用途或结论；
- `problem` 必须明确它用于识别、解决、避免或判断什么问题，不能只用问句充数；
- `inputOutput` 必须同时交代已知输入和产生的输出、结果或判断；
- `mechanism` 必须给出步骤、变换或因果关系，不能把“选择某结果”冒充工作原理；
- `interpretation` 必须把可观察结果映射到含义，说明什么现象支持什么判断；
- `boundary` 必须写出前提、限制、失败条件或不能推出的结论。

合同答案写得比正文完整、图中只有轴标签、证据只是章节问题或结果短语，都会产生 `section.insufficient-*` 阻断。该规则专门防止“元数据声称六问已经回答，读者正文却看不到解释”的假阳性。

“输入和输出”按章节对象解释：算法章节是数据到结果，评测章节是候选结果到评估证据，概念章节是已知条件到可作出的判断。该规则不要求每节都有公式或代码。`synthesis`、`assessment` 和 `reference` 使用各自门禁，不机械套用六问。

### 5.4 六问融合检查

六问是对正文覆盖情况的审计，不是要求正文展示六个整齐答案。门禁使用两类高置信
阻断：

- `integration.tail-evidence-cluster`：至少五项审计证据集中在章节后半部的短窗口；
- `integration.append-only-contract-fix`：相对 Git 基线，原正文基本未改，新增尾部承载至少四项审计证据。

位置判断基于去除标签后的可见文本，因此把一个收束段拆成多个 `<p>`、列表或提示框
不会改变结果。证据紧凑但不足以高置信判错时，只产生
`editorial.contract-like-evidence-cluster`，交由人工判断。任何缺口都应通过重写原有
教学过程解决，禁止追加审计答案。
...
- 相邻核心章节连续使用“X 是 Y”“X 是一种 Y”等同构定义模板，形成合同腔；定义仍须清楚，但应允许“可以理解为、指的是、描述了、所做的是、比较的是”等自然变体；
- 类比没有说明对应关系和失效边界；
- “必须、总是、等于正确答案”等绝对化表述；
- 总体量、经验估计量和因果结论可能混淆；
...
这些提示必须进入 L4审校，不得转化成关键词加分。定义句式重复只有在形成明显模板化段落时才提示；单次使用“X 是 Y”完全合格，不得因此产生 L3 硬阻断。当前定义句式提示使用 `editorial.repeated-definition-template`，只作为候选线索并交给人工确认。
