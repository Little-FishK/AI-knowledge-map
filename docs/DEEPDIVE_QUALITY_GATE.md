...
这些提示必须进入 L4审校，不得转化成关键词加分。定义句式重复只有在形成明显模板化段落时才提示；单次使用“X 是 Y”完全合格，不得因此产生 L3 硬阻断。未来若增加自动提示，规则 ID 使用 `style.repetitive-definition-template`，并至少以三个相邻 `core` 章节的同构开头作为候选线索，再交给人工确认。
