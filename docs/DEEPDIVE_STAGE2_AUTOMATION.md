...
| 人工审查恢复 | `tools/deepdive-stage2/lib/review-recovery.js` | 将人工复审或已自动通过页面安全重置到独立审计队列，并清理对应旧结论 |
| 人工审查预览 | `tools/deepdive-stage2/lib/manual-review-preview.js` | 生成候选与正式页的字段、章节和两轮审查差异预览，并清理不安全 HTML |
...
现有页面与新节点都发布到 `data/deepdive/<pageId>.js` 的唯一完整规范源。控制器以原子写入、写前哈希和失败回滚保护该文件；不再生成末位覆盖文件，也不存在承载多个页面的共享批次源。
