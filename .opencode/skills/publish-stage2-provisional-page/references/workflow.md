# Complete page-locked workflow (opencode)
...
In opencode, workers are Task-tool subagents (`stage2-content-generation`, `stage2-audit`, `stage2-repair`). Each worker is page-locked by its task prompt, role-locked by its agent definition, single-claim, single-submit, and terminal after submission.
...
5. Spawn one fresh `stage2-content-generation` subagent locked to this page. Give it only this compact task: claim this `pageId` once, process every eligible chapter in order, save each complete response before reading the next, submit exactly one result, report the controller response, and stop. Do not give it context from another node and do not reuse it later for another node.
6. The content subagent calls `stage2_claim_task` once with this `pageId`. If the result is `paused`, `busy`, or `idle`, it stops without claiming again.
7. The controller resolves the old chapter source only from `docs/deepdive-reviews/<leading-order-digit>x-section-text-review.md`. The subagent must not open that file itself. It receives only an ordered eligible-section list.
8. For each eligible section in source order, the same subagent calls `stage2_read_content_section` once for that section, then answers using exactly this prompt and no replacement prompt:
...
9. The subagent immediately saves the complete response with `stage2_save_content_response`. It may read the next section only after save succeeds.
10. Never request, read, summarize, or save sections titled `常见误解`, `自测`, or `检查你是否真的理解`. The controller also rejects them.
11. After all eligible sections are saved, the subagent calls `stage2_submit_result` exactly once and stops. The controller atomically writes `docs/deepdive-reviews/<pageId>-agent-responses.md`, clears the lease, and restores the pre-generation workflow state.
...
17. For `audit-queued`, spawn one fresh `stage2-audit` subagent locked to this page. It claims once, reads its contract and candidate only through lease-bound Stage 2 tools, performs the whole-page factual/concept/relationship/style/formula audit, validates its audit result, submits once, and stops. It must not write page content or reuse a prior audit answer.
18. If the valid audit has no blocker, proceed to `manual-review`. If it yields sanitized content blockers, the controller may queue one `stage2-repair` subagent. The repair subagent claims once, uses only its sanitized defects and current candidate, validates and submits one complete repaired page, and stops. It must not search for the private audit answer or invent audit evidence.
...
- opencode has no Codex task archival tool; skipping archival is not a Stage 2 state change.
- A scheduled invocation processes only this page and then ends, even if another page is ready.

## opencode deployment notes

- The `stage2` MCP entry runs the server with the `full` capability profile (no `STAGE2_MCP_PROFILE` env), so every tool is visible to every agent. The page-lock and per-role tool allowlists that Codex profiles enforce are therefore soft in opencode: enforce them through the worker agent definitions (no filesystem, no shell, no web except audit) and through this contract, and rely on the controller state machine for single-lease, single-claim, hash binding, and publication authority.
- The page lock is applied per-worker via the task prompt: each worker subagent receives exactly one `pageId` and must never claim or touch another page.
- Controller-only tools (`stage2_finalize_manual_review`, `stage2_rollback_editorial_candidate`, `stage2_rollback_provisional_page`, `stage2_release_lease`, `stage2_reset_manual_review`, `stage2_reset_passed_page`, `stage2_create_manual_review_preview`) may only be invoked by the primary controller agent, never by a worker subagent.
- 本部署与 Codex 部署共存：Codex 端继续使用 `~/.codex/skills/` 与 `.codex/agents/` 原文件，本文件与其保持内容同步（以 `docs/DEEPDIVE_STAGE2_AUTOMATION.md` 为事实源）。
