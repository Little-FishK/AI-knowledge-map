...
<section class="dd-sec" data-section-role="core">
  ...
  <p>分类与回归描述的是<b>监督学习输出的两种基本形态</b>，用来避免把类别、概率和连续数值混用。给定样本特征和训练目标，分类输出类别或各类别概率，回归输出一个连续数值；训练时都先产生预测，再用与任务匹配的损失比较预测和目标，最后调整模型。</p>
...
<section class="dd-sec" data-section-role="core">
  ...
  <p>泛化评估描述的是<b>模型在未参与训练和选择的数据上是否仍然有效</b>，它解决训练成绩无法代表真实使用效果的问题。输入是冻结的模型、独立样本及其目标，输出是损失、精确率、召回率和业务切片等验收证据。</p>
  <p>评估时先用训练集拟合参数，再用验证集选择方案，最后只在方案冻结后使用测试集。验证误差开始上升而训练误差继续下降，说明模型更像在记忆训练细节；但一次测试高分仍受样本代表性、时间变化和数据泄漏影响，不能证明所有未来场景都可靠。</p>
</section>
...
<section class="dd-sec" data-section-role="core">
  ...
  <p>“标注瓶颈”记录的是<b>可靠目标信号在成本、质量和覆盖上的限制</b>，用来判断一个监督方案能否支撑真实部署。评估时输入标注来源、抽样范围、一致性结果和部署场景，输出可用标签规模、噪声切片、覆盖缺口与继续采集的优先级。</p>
...
<section class="dd-sec" data-section-role="core">
  ...
  <p>这张比较表描述的是<b>训练信号从哪里来、学习后产出什么</b>，用于在没有人工标签时选择合适范式。输入是任务可获得的数据、反馈方式和交互条件，输出是对监督、无监督、自监督或强化学习路线的选择，以及对应的模型、结构表示或策略。</p>
...
<section class="dd-sec" data-section-role="core">
  ...
  <p>大模型时代的监督学习角色，可以理解为<b>用高质量目标信号约束通用模型的具体行为</b>。它解决的是预训练能力很广却未必按任务要求作答的问题：输入可以是指令与理想回答、领域样本与标签，输出是经过监督微调的模型或面向具体任务的预测器。</p>
