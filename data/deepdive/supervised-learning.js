    ...
    "sectionContracts": [
      {
        "section": 1,
        "definition": {
          "answer": "监督学习描述利用配对的输入和目标样本来学习预测规则的训练范式。",
          "evidence": "利用“输入—目标”样本学习预测规则"
        },
        "problem": {
          "answer": "它用于解决怎样根据过去样本对未见输入作出可检验预测的问题。",
          "evidence": "面对一封训练时没见过的新邮件，怎样依据过去样本给出可检验的判断"
        },
        "inputOutput": {
          "answer": "输入是邮件及其标签，输出是能够对新邮件产生类别或数值预测的模型。",
          "evidence": "邮件内容与对应标签"
        },
        "mechanism": {
          "answer": "训练先产生预测，再与标签比较，然后根据误差调整规则并反复执行。",
          "evidence": "先让模型预测训练样本，再把预测与标签比较"
        },
        "interpretation": {
          "answer": "垃圾概率零点八表示当前模型更偏向垃圾类别，不表示标签或结论必然正确。",
          "evidence": "表示它在当前规则下更偏向垃圾类别"
        },
        "boundary": {
          "answer": "标签可能错误，换到另一邮件来源时模型也可能不再可靠。",
          "evidence": "不能证明标签绝对正确"
        }
      },
      {
        "section": 2,
        "definition": {
          "answer": "分类与回归描述监督学习输出的两种基本形态：离散类别与连续数值。",
          "evidence": "监督学习输出的两种基本形态"
        },
        "problem": {
          "answer": "这种区分用于避免把类别、类别概率和连续数值混作同一种预测目标。",
          "evidence": "用来避免把类别、概率和连续数值混用"
        },
        "inputOutput": {
          "answer": "输入是样本特征和训练目标；分类输出类别或概率，回归输出连续数值。",
          "evidence": "分类输出类别或各类别概率"
        },
        "mechanism": {
          "answer": "两类任务都先预测，再用匹配的损失比较目标，最后调整模型。",
          "evidence": "训练时都先产生预测，再用与任务匹配的损失比较预测和目标"
        },
        "interpretation": {
          "answer": "分类结果说明样本更符合哪个类别，回归结果说明估计数值的大小。",
          "evidence": "分类结果说明样本更符合哪个预先定义的类别"
        },
        "boundary": {
          "answer": "任务类型不能只按答案表面是数字还是文字决定，还取决于预测对象和错误代价。",
          "evidence": "不能只凭“看起来像数字还是文字”机械决定任务类型"
        }
      },
      {
        "section": 3,
        "definition": {
          "answer": "参数学习指的是从候选参数中寻找训练样本平均损失较小的一组参数。",
          "evidence": "从候选参数中寻找平均损失较小的一组"
        },
        "problem": {
          "answer": "它解决如何把预测与标签的不一致转化成可优化目标的问题。",
          "evidence": "让模型在训练样本上的平均错误尽量小"
        },
        "inputOutput": {
          "answer": "输入是训练样本、当前参数和损失规则，输出是更新参数及其预测函数。",
          "evidence": "训练样本、当前参数和损失规则"
        },
        "mechanism": {
          "answer": "训练先计算预测，再计算样本损失，然后汇总平均值并更新参数。",
          "evidence": "先用当前参数计算预测，再计算每个样本的损失"
        },
        "interpretation": {
          "answer": "平均损失下降表示当前样本上更贴近标签，不表示所有样本都正确或已经泛化。",
          "evidence": "平均损失下降表示模型在当前样本和损失定义下更贴近标签"
        },
        "boundary": {
          "answer": "二分类小批次只解释概率到损失，不能替代梯度推导和独立测试。",
          "evidence": "不能替代后续的梯度推导或测试集验收"
        }
      },
      {
        "section": 4,
        "definition": {
          "answer": "泛化评估描述模型在未参与训练与选择的数据上是否依然有效。",
          "evidence": "模型在未参与训练和选择的数据上是否仍然有效"
        },
        "problem": {
          "answer": "它解决训练集成绩无法代表模型真实使用效果的问题。",
          "evidence": "它解决训练成绩无法代表真实使用效果的问题"
        },
        "inputOutput": {
          "answer": "输入是冻结模型、独立样本及目标，输出是损失、精确率、召回率和切片证据。",
          "evidence": "冻结的模型、独立样本及其目标"
        },
        "mechanism": {
          "answer": "流程先训练参数，再用验证集选方案，最后在方案冻结后使用测试集。",
          "evidence": "先用训练集拟合参数，再用验证集选择方案"
        },
        "interpretation": {
          "answer": "训练误差下降而验证误差上升，说明模型更像在记忆训练细节。",
          "evidence": "说明模型更像在记忆训练细节"
        },
        "boundary": {
          "answer": "一次高分仍依赖样本代表性、时间稳定性和无泄漏前提，不能证明未来都可靠。",
          "evidence": "一次测试高分仍受样本代表性、时间变化和数据泄漏影响"
        }
      },
      {
        "section": 5,
        "definition": {
          "answer": "标注瓶颈记录可靠目标信号在成本、质量和覆盖范围上的限制。",
          "evidence": "可靠目标信号在成本、质量和覆盖上的限制"
        },
        "problem": {
          "answer": "它用于判断一个监督数据方案能否支撑真实部署，而不只看标签数量。",
          "evidence": "用来判断一个监督方案能否支撑真实部署"
        },
        "inputOutput": {
          "answer": "输入是标注来源、抽样范围、一致性和部署场景，输出是噪声与覆盖缺口。",
          "evidence": "标注来源、抽样范围、一致性结果和部署场景"
        },
        "mechanism": {
          "answer": "先抽样复核一致性，再比较人群、时间和来源覆盖，随后检查目标是否一致。",
          "evidence": "先抽样复核标注一致性，再比较不同人群、时间和来源的覆盖"
        },
        "interpretation": {
          "answer": "高一致率表示标注规则较稳定，但不表示目标本身没有偏差。",
          "evidence": "高一致率表示标注规则较稳定"
        },
        "boundary": {
          "answer": "自动日志便宜但可能不可信，专家复核准确却可能昂贵缓慢。",
          "evidence": "自动日志便宜也不代表可信"
        }
      },
      {
        "section": 6,
        "definition": {
          "answer": "四种范式比较描述训练信号的来源以及学习过程最终产出的对象。",
          "evidence": "训练信号从哪里来、学习后产出什么"
        },
        "problem": {
          "answer": "它用于在缺少人工标签时选择与数据和反馈条件相符的学习范式。",
          "evidence": "用于在没有人工标签时选择合适范式"
        },
        "inputOutput": {
          "answer": "输入是可用数据、反馈和交互条件，输出是学习路线及对应模型、表示或策略。",
          "evidence": "任务可获得的数据、反馈方式和交互条件"
        },
        "mechanism": {
          "answer": "判断先看外部目标，再看能否自造目标，最后看是否需要行动获得延迟奖励。",
          "evidence": "先看是否有外部目标值，再看数据能否自行构造目标"
        },
        "interpretation": {
          "answer": "范式名称表示主要监督来源，不表示一个实际系统只能使用其中一种。",
          "evidence": "分类结果说明主要监督来源"
        },
        "boundary": {
          "answer": "自监督和强化学习系统都可能组合监督训练，传统范式边界并非绝对互斥。",
          "evidence": "边界允许组合"
        }
      },
      {
        "section": 7,
        "definition": {
          "answer": "大模型时代的监督学习可以理解为用高质量目标信号约束通用模型行为。",
          "evidence": "用高质量目标信号约束通用模型的具体行为"
        },
        "problem": {
          "answer": "它解决预训练模型能力广泛却未必按照具体任务要求作答的问题。",
          "evidence": "它解决的是预训练能力很广却未必按任务要求作答的问题"
        },
        "inputOutput": {
          "answer": "输入是指令与理想回答或领域样本与标签，输出是监督微调模型或任务预测器。",
          "evidence": "指令与理想回答、领域样本与标签"
        },
        "mechanism": {
          "answer": "应用先定义行为，再收集输入目标对，然后最小化损失并在独立切片验收。",
          "evidence": "先定义希望模型表现出的行为，再收集有代表性的输入—目标对"
        },
        "interpretation": {
          "answer": "下游分数提高说明模型更适合当前目标和分布，不能说明知识全面增加。",
          "evidence": "下游分数提高说明模型更适合当前目标和分布"
        },
        "boundary": {
          "answer": "示范狭窄、标签有偏或部署任务变化时，监督适配仍可能失效。",
          "evidence": "示范数据狭窄、标签带偏或部署任务变化时"
        }
      }
    ],
    "formulas": [
      ...
      },
      {
        ...
        "symbols": [
          {
            "name": "L",
            "meaning": "当前样本的二分类交叉熵损失",
            "evidence": "二分类交叉熵写成完整形式"
          },
          {
            "name": "y",
            "meaning": "垃圾邮件取一、正常邮件取零的标签",
            "evidence": "y=1 表示垃圾邮件"
          },
          {
            "name": "p",
            "meaning": "模型输出的垃圾邮件概率",
            "evidence": "这封邮件是垃圾邮件」的概率"
          }
        ]
      }
    ]
