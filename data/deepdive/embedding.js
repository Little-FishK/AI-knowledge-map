...
<section class="dd-sec">
  ...
  <p><b>嵌入是一种把文本映射成连续向量的表示方法。</b>机器只会算数字，不会直接比「意思」。所以第一步是把文本变成数字——但不是随便变，而要变得<b>让「意思近」体现成「数字近」</b>。一段文本经过嵌入模型，变成一个几百到几千维的<b>向量</b>，且满足一条关键性质——<b>语义相近的文本，向量之间的距离也近</b>。可以把它想成给每段话在一张「语义地图」上标了个坐标。嵌入用于解决机器无法直接比较文字意思、只能算数字这一问题；它输入一段文本，先经嵌入模型编码，再输出一个几百到几千维的向量。两段文本向量距离越近，表示它们语义越相近，但这种距离只在同一嵌入模型的空间里可比，换模型就不成立。</p>
  <figure class="dd-fig">
...
<section class="dd-sec">
  ...
  <p>不是碰巧，是<b>训练目标逼出来的</b>。训练嵌入模型时，喂给它大量「这两句意思相近 / 这两句无关」的样本对，让它<b>调整参数，使相近的向量靠拢、无关的向量推开</b>。反复很多遍之后，整个向量空间的几何结构，就把语义关系「学」进去了。距离等于语义可以理解为训练目标塑造出来的几何性质，用于解决凭什么向量远近能对应意思远近这一疑问；它输入大量相近或无关的样本对，先据配对调整参数，再让相近向量靠拢、无关向量推开，输出一个语义被编码进几何的向量空间。相近样本被拉近表示模型学到了它们的关联，但这只是训练配对逼出的副产品，不是设计的规则。</p>
...
<section class="dd-sec">
  ...
  <p><b>余弦相似度用两个向量夹角的余弦值比较方向。</b>它最重要的用途是判断两段话是不是在讲同一件事：<b>不再需要它们共享字面词汇</b>，只要比较两个向量的夹角——夹角越小、越同向，就越相近。语义，第一次变成了能用一个数直接比较的量。余弦相似度可以理解为用夹角余弦衡量两个向量方向接近程度的指标，用于解决怎样把两段话是否同义变成一个可比数值的问题；它输入两个嵌入向量，先算点积再除以两者长度，输出一个 −1 到 1 的相似度分数。余弦值越大越表示两者方向越接近、语义越相近。</p>
...
<section class="dd-sec">
  ...
  <table class="dd-table"><thead><tr><th>候选</th><th>点积</th><th>余弦与结论</th></tr></thead><tbody><tr><td>a</td><td><code>0.8</code></td><td><code>0.8</code>，更相关</td></tr><tr><td>b</td><td><code>−0.6</code></td><td><code>−0.6</code>，更不相关</td></tr></tbody></table><p>展开计算分别是 <code>1×0.8 + 0×0.6 = 0.8</code> 与 <code>1×(−0.6) + 0×0.8 = −0.6</code>；因为三个向量都是单位长度，分母均为 1。这次手算描述余弦相似度如何把方向差异变成一个可排序的分数，用于解决更同向怎样落成可复算排名的问题；它输入查询与候选向量，先做点积、再按长度归一，输出每个候选的相似度。分数越高表示方向越接近，但数值高只是模型空间内的相似，不代表业务上正确。</p>
...
<section class="dd-sec">
  ...
  <div class="dd-note warn"><b>为什么要分清</b>　两者名字都叫「嵌入」，但一个是把 token 送进模型的<b>第一层</b>，一个是产出「整段文本表示」的<b>专用模型</b>。做 RAG 时你需要的是后者；把两者混为一谈，选型和实现都会出错。这里比较的是两个同名却不同的嵌入，用于解决该用哪一个的选型问题；输入同样是文本，但输入嵌入层先给每个 token 一个向量、再送进 Transformer，而嵌入模型直接输出整段文本的一个向量。用途不同表示不能互相替代，但两者都叫嵌入，混用会导致选型和实现出错。</div>
</section>
...
<section class="dd-sec">
  ...
  <p>关键词搜索靠<b>字面匹配</b>：查询词必须和文档里的词对上。所以查「怎么退货」，命中不了只写「商品返还流程」的文档。而语义搜索换了个思路：把查询和每篇文档都<b>嵌入成向量</b>，再找<b>离查询向量最近</b>的那些文档——匹配的是<b>意思</b>，不是字。于是「退货」和「返还流程」尽管没有共同词，也能被召回。语义搜索可以理解为按意思而非字面找最近邻的检索方式，用于解决关键词搜索漏掉没有共同词的相关文档的问题；它输入查询和文档库，先把两者都嵌入成向量，再输出离查询最近的文档。某文档被召回表示它与查询意思相近，但召回质量依赖嵌入好坏，好的平均召回也不保证每条都对。</p>
...
<section class="dd-sec">
  ...
  <p class="dd-lead">用嵌入最容易在哪翻车？两个坑几乎人人踩过。认识这两个坑用于避免嵌入在工程落地时最常见的两类翻车；这两个坑描述换模型和否定两种失败，它们的共同输入是待检索文本和所用嵌入模型，输出是检索结果的可靠性；第一个坑源于换模型后向量先落到不同空间、再被拿来混用，第二个坑源于否定词几乎不改变向量。某次检索召回对不对，表示的是当前模型空间内的相似，并不代表语义真的正确。</p>
  <ul class="dd-steps">
