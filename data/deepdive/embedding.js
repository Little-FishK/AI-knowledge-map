...
<section class="dd-sec">
  ...
  <p><b>嵌入是一种把文本映射成连续向量的表示方法。</b>机器只会算数字，不会直接比「意思」。所以第一步是把文本变成数字——但不是随便变，而要变得<b>让「意思近」体现成「数字近」</b>。一段文本经过嵌入模型，变成一个几百到几千维的<b>向量</b>，且满足一条关键性质——<b>语义相近的文本，向量之间的距离也近</b>。可以把它想成给每段话在一张「语义地图」上标了个坐标。</p>
  <figure class="dd-fig">
...
<section class="dd-sec">
  ...
  <p><b>余弦相似度用两个向量夹角的余弦值比较方向。</b>它最重要的用途是判断两段话是不是在讲同一件事：<b>不再需要它们共享字面词汇</b>，只要比较两个向量的夹角——夹角越小、越同向，就越相近。语义，第一次变成了能用一个数直接比较的量。</p>
