...
// 新版教学门禁补充：逐节说明代码生成的输入、输出、机制、解释与边界。
{
  const page = window.DEEPDIVE["code-generation"];
  const additions = [
    '<p>AI 编程是把自然语言意图转换为可运行代码的生成任务。输入是需求、已有代码和验收条件，输出是候选补丁、解释或新程序。它从补全到跨文件实现都使用同一机制；代码能够运行只表示语法和部分路径成立，不证明需求、安全与性能全部正确。</p>',
    '<p>代码适合大模型，是因为训练语料丰富、语法结构重复且可以用编译和测试产生外部反馈。输入是需求与代码模式，输出是可被工具检验的候选实现。验证把模糊错误压成具体反例，但只覆盖已表达规格；缺失边界条件仍会让测试全绿的错误代码通过。</p>',
    '<p>能力阶段输入任务范围、仓库上下文、工具和自主权，输出补全、对话修改、仓库级补丁或 Agent 轨迹。范围扩大时系统增加搜索、规划、执行和恢复闭环。阶段更高表示负责的工作面更宽，不表示模型本身更聪明，也不应自动获得更大权限。</p>',
    '<p>代码执行示例输入缓存需求、首个补丁、用户隔离测试和项目回归，输出修正后的最小补丁与运行证据。系统先把需求变成三条验收，再用失败测试定位缓存键缺少 user_id，修复后由窄到宽验证。测试通过只支持已覆盖行为，不授权自动合并或发布。</p>',
    '<p>仓库上下文工程输入当前失败、接口、调用者、测试、约定和依赖版本，输出完成修改所需的最小相关代码包。系统从复现和符号关系逐步扩展材料，避免把整个仓库无差别塞入窗口。上下文充分只表示关键约束可见，模型仍可能误解或遗漏未加载路径。</p>',
    '<p>代码风险控制输入候选补丁、依赖、测试、安全规则和审查者，输出接受、修正、拒绝或待验证项。编译、测试、静态分析和人工 diff 分层检查幻觉 API、边界 bug 与漏洞。把模型当高产初级工程师是责任分配，不表示人类审查天然不会漏错。</p>',
    '<p>代码生成评测输入固定任务、隐藏测试、工具权限、采样数 k 和重试预算，输出 pass@1、pass@k 或仓库级任务成功。pass@k 统计 k 个候选至少一个命中，仓库评测还要求定位、最小修改和回归。分数只在预算和环境一致时可比，测试通过不能证明安全性能和完整意图。</p>'
  ];
  const renderedSections = page.html.split("</section>");
  additions.forEach((html, index) => { renderedSections[index] += html; });
  page.html = renderedSections.join("</section>").replace(
    '<h2><span class="dd-n">4</span>关键搭档：代码执行',
    '<h2><span class="dd-n">4</span>关键搭档：代码执行运行示例'
  );
}
