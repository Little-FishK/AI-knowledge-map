...
// 新版教学门禁补充：逐节说明 CoT 的输入输出、机制、触发、变体和忠实性边界。
{
  ...
  const additions = [
    '<p>思维链可以理解为让模型先写出一步步推理、最后再给结论的提示方式，用于解决模型不假思索、一步就答错难题的问题；它输入一个多步问题和分步指令或示例，先把直接猜结论改成逐步写出可检查的局部状态，再据此得出答案，输出中间步骤与最终答案。写出步骤通常表示难题被拆成更容易接对的小步，但中间文字仍可能出错或是事后合理化。</p>',
    '<p>思维链之所以有效，可以理解为给模型提供了一张外显草稿纸，用于解决为什么写出来比不写更准的疑问；它输入逐词生成、无全局草稿这一事实，先把一步答难题拆成一串小步，再让已写步骤成为后续生成的上下文，输出更稳的分步推理。可检查的中间步骤表示错误位置能被看见，但语言步骤本身仍可能写错，能算的最好交给计算器或代码验证。</p>',
    '<p>触发方式可以理解为让模型产生思维链而非直接蹦答案的几种做法，用于解决怎样稳定引出中间步骤的问题；它输入任务、模型能力、示例数量和输出要求，先给一两个带步骤的示例让模型模仿，再改成只要求逐步处理，输出直接答案或分步回答。能被示例触发说明模型在做上下文学习，但高风险结果仍需计算器、代码或来源验证。</p>',
    '<p>进阶变体可以理解为在单条思维链之上用更多计算换更稳答案的方法，用于解决一条链偶然出错、想更可靠的问题；它输入单链结果、采样预算和答案聚合或搜索评价规则，先多次独立采样再按答案投票，或展开多分支评估并回溯，输出自洽投票或思维树搜索结果。多条路径一致通常表示答案更可信，但候选高度相关或评价器失准时，多想仍可能一致地错。</p>',
    '<p>这一节比较的是思维链与推理模型两个容易混的概念，用于解决它们到底什么关系的问题；它输入能力来源、预算控制、计算形态和轨迹可见性，先看能力是靠提示引出还是靠后训练学得，再看轨迹是否展示，输出对二者关系的判断。都用到更多推理表示两者相关，但不能用是否写出长过程来等同，推理模型的原始轨迹还可以隐藏。</p>',
    '<p>这一节描述思维链的使用代价与取舍，用于解决什么时候这份草稿纸反而不值得的问题；它输入任务步数、可验证性、延迟费用、上下文预算和风险，先判断题目是否真的多步、可否外部验证，再决定直接答、分步生成还是转工具，输出一个使用决策。输出变长通常表示更慢更贵、还挤占窗口，但简单题强行分步只会啰嗦，可见思维链既不保证正确、也不是可靠的可解释性证据。</p>'
  ];
