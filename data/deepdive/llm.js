...
<section class="dd-sec">
  ...
  <p>第二条路径的第二步更“顺”，仍补不回第一步的巨大差距。真实生成拥有指数多条路径，贪心只看当前一步，束搜索近似比较多条累计路径，随机采样则按每一步分布探索；这三者不能混称为“模型的答案”。</p>
</section>
...
<section class="dd-sec">
  ...
  <p>语言似然不等于事实真值，确实解释了幻觉风险和为什么需要外部证据；但 RAG 还解决知识更新与私有数据，对齐处理偏好和安全目标，提示注入则源于指令与不可信数据共享同一输入通道。它们相关，却不是完全相同的单一根因。</p>
</section>
