    ...
    "examples": [
      {
        "section": 2,
        "evidence": {
          "setup": "2 它要同时办成两件事 工程 「一句话 → 一张图」",
          "rule": "中间其实要跨过两道坎",
          "steps": "两阶段生成描述了从“理解要求”到“形成画面”的完整过程",
          "interpretation": "用于解决文字含义与图像像素之间不能直接一一对应的问题"
        }
      },
      {
        "section": 4,
        "evidence": {
          "setup": "怎么让它更听话 案例推演 只给一句话",
          "rule": "控制太粗——想要精确的构图、姿势、局部修改",
          "steps": "可控生成所做的",
          "interpretation": "是把结构约束加到文生图过程里"
        }
      }
    ],
    "termReviews": [],
    "sectionContracts": [
      {
        "section": 1,
        "definition": {
          "answer": "图像生成是一种按照文字、参考图等条件创造图像候选的任务，不是图库检索。",
          "evidence": "所谓图像生成，可以理解为“按条件创造图像候选”"
        },
        "problem": {
          "answer": "它解决没有现成图片时，如何得到符合描述的新画面这一问题。",
          "evidence": "它解决的是“没有现成图片，却希望得到符合描述的新画面”这一问题"
        },
        "inputOutput": {
          "answer": "输入是文字及可选的参考图、草图或姿势，输出是一张或多张像素图像。",
          "evidence": "参考图、草图或姿势"
        },
        "mechanism": {
          "answer": "系统先编码输入条件，再采样新的内部表示，最后把该表示解码为可见图像。",
          "evidence": "系统先把输入条件转换成内部表示，再根据训练中学到的图像规律采样一个新表示"
        },
        "interpretation": {
          "answer": "结果只表示模型认为画面与条件相容，不能证明画面中的事件真实发生。",
          "evidence": "生成结果表示“模型认为这种画面与条件相容”"
        },
        "boundary": {
          "answer": "模型可能复现训练片段，生成新图不能证明绝对原创、事实正确或可无限制使用。",
          "evidence": "模型仍可能记住并复现训练片段"
        }
      },
      {
        "section": 2,
        "definition": {
          "answer": "两阶段生成是一个把文字要求先变成条件表示、再形成画面的处理流程。",
          "evidence": "两阶段生成描述了从“理解要求”到“形成画面”的完整过程"
        },
        "problem": {
          "answer": "它用于解决文字含义与图像像素之间不能直接一一对应的问题。",
          "evidence": "用于解决文字含义与图像像素之间不能直接一一对应的问题"
        },
        "inputOutput": {
          "answer": "输入是提示词和可选控制条件，中间输出是条件表示，最终输出是图像候选。",
          "evidence": "提示词和可选控制条件"
        },
        "mechanism": {
          "answer": "系统先编码文字，再由生成引擎形成图像表示，然后解码并逐项验收。",
          "evidence": "系统先把“柯基、帽子、海边、油画”编码成条件表示，再让生成引擎依照这些条件形成图像表示"
        },
        "interpretation": {
          "answer": "整体相似只说明大方向可能对齐，不能证明对象数量与空间关系都正确。",
          "evidence": "某个候选整体上很像提示词，只说明大方向可能对齐"
        },
        "boundary": {
          "answer": "两阶段图示用于建立直觉，但不同模型不一定采用完全相同的固定部件。",
          "evidence": "不同模型会把编码、生成和解码组合成不同结构"
        }
      },
      {
        "section": 3,
        "definition": {
          "answer": "扩散模型是一种通过反复去噪来生成数据的图像生成引擎。",
          "evidence": "扩散模型指的是通过反复去噪来生成数据的引擎"
        },
        "problem": {
          "answer": "它解决如何从随机起点得到既自然又符合文字条件的图像。",
          "evidence": "它解决的是“怎样从随机起点得到既自然又符合文字要求的图像”"
        },
        "inputOutput": {
          "answer": "输入是随机噪声、去噪步数和文字条件，每一步输出更有结构的状态，最终输出图片。",
          "evidence": "随机噪声、当前去噪步数和文字条件"
        },
        "mechanism": {
          "answer": "系统先预测符合目标的变化，再去掉部分噪声，然后重复处理直至形成画面。",
          "evidence": "系统先预测当前噪声中哪些变化更像目标画面，再去掉一部分噪声"
        },
        "interpretation": {
          "answer": "一次结果表示某个随机起点下的候选，并不是提示词的唯一标准答案。",
          "evidence": "一次运行得到的是随机起点下的一个候选"
        },
        "boundary": {
          "answer": "更多步骤不能保证计数、文字或事实正确，扩散也不是图像生成的唯一路线。",
          "evidence": "增加去噪步数不一定持续提升质量"
        }
      },
      {
        "section": 4,
        "definition": {
          "answer": "可控生成是给文生图增加姿势、深度、线稿等结构约束的工作方式。",
          "evidence": "可控生成所做的，是把结构约束加到文生图过程里"
        },
        "problem": {
          "answer": "它用于解决提示词正确但位置、姿势或局部细节仍不可控的问题。",
          "evidence": "用于解决“文字说得对，但位置、姿势或局部细节仍不可控”的问题"
        },
        "inputOutput": {
          "answer": "输入是提示词与结构条件或遮罩，输出是同时服从语义与结构约束的图像。",
          "evidence": "姿势骨架、深度图、线稿或局部遮罩"
        },
        "mechanism": {
          "answer": "流程先拆分条件，再约束构图，然后生成候选，最后局部重绘错误区域。",
          "evidence": "先把模糊愿望拆成可检查条件，再用结构图约束整体构图"
        },
        "interpretation": {
          "answer": "若多个候选在同处失败，说明更像条件不清、约束冲突或模型覆盖不足。",
          "evidence": "若四个候选都在同一位置失败，更像是条件表达不清、约束冲突或模型没有学好这种组合"
        },
        "boundary": {
          "answer": "冲突条件无法同时完全服从，局部重绘也可能意外改变邻近区域。",
          "evidence": "控制条件越多并不一定越好"
        }
      },
      {
        "section": 5,
        "definition": {
          "answer": "能力边界评测是一种按照失败类型系统检查模型的测试方法。",
          "evidence": "能力边界评测记录模型在不同失败类型上的表现"
        },
        "problem": {
          "answer": "它用于发现展示图无法揭示的、真实任务中可重复出现的能力短板。",
          "evidence": "用于解决“展示图很好看，却不知道系统在真实任务中会在哪里失灵”的问题"
        },
        "inputOutput": {
          "answer": "输入是分维度测试提示和采样次数，输出是各类错误的成功率、失败率与案例。",
          "evidence": "覆盖文字、计数、空间关系、一致性和事实性的测试提示"
        },
        "mechanism": {
          "answer": "评测先设计固定提示，再统一重复生成，然后按规则标错，最后比较维度和版本。",
          "evidence": "先为每个高风险维度设计一组固定提示，再让同一模型按统一分辨率和采样预算重复生成"
        },
        "interpretation": {
          "answer": "单张成功只说明该次采样成功，重复失败才说明存在稳定的能力短板。",
          "evidence": "看到一张成功图，只说明该次采样成功"
        },
        "boundary": {
          "answer": "清单不能永久代表所有模型；版本、语言、画幅或工具变化后必须重新测量。",
          "evidence": "这份清单不能永久代表所有模型"
        }
      },
      {
        "section": 6,
        "definition": {
          "answer": "多维评估是一套把模糊的图像质量拆成多个独立问题的验收方法。",
          "evidence": "多维评估比较的是“质量”的不同侧面，而不是寻找一个万能总分"
        },
        "problem": {
          "answer": "它用于避免好看的画面掩盖贴题错误、身份漂移和安全风险。",
          "evidence": "用于避免一张好看的图掩盖贴题错误、身份漂移或安全风险"
        },
        "inputOutput": {
          "answer": "输入是固定提示、模型版本、种子、预算和评分规则，输出是分维度结果与失败案例。",
          "evidence": "固定提示集、模型版本、随机种子策略、采样预算和人工评分规则"
        },
        "mechanism": {
          "answer": "评估先固定测试条件，再重复生成，然后自动筛查并人工盲评，最后汇总风险。",
          "evidence": "先固定提示、分辨率和采样次数，再对每条提示重复生成"
        },
        "interpretation": {
          "answer": "某维度更高只表示该维度在当前测试条件下更好，不能推出整体更好。",
          "evidence": "某个维度更高，只表示该维度在这组测试条件下更好"
        },
        "boundary": {
          "answer": "自动指标不覆盖全部语义，人工评价也有偏差，安全失败不能被高分抵消。",
          "evidence": "自动指标难覆盖构图偏好和高风险语义"
        }
      },
      {
        "section": 7,
        "definition": {
          "answer": "生成图像治理是一套覆盖生成前、生成中与发布后的风险控制流程。",
          "evidence": "生成图像治理，可以理解为贯穿生成前、生成中和发布后的风险控制"
        },
        "problem": {
          "answer": "它用于降低版权侵害、冒充真人、诈骗和虚假传播等风险。",
          "evidence": "用于降低版权侵害、冒充真人、诈骗和虚假传播"
        },
        "inputOutput": {
          "answer": "输入是请求、人物与素材、授权和用途，输出是允许、限制、拒绝和复核记录。",
          "evidence": "用户请求、涉及的人物与素材、授权状态和使用场景"
        },
        "mechanism": {
          "answer": "流程先核查身份授权，再限制高风险生成，然后记录来源并人工复核敏感内容。",
          "evidence": "生成前先检查身份、授权与用途，生成时限制明显高风险请求"
        },
        "interpretation": {
          "answer": "水印只表示某套工具留下来源线索，检测器的结果也只是概率判断。",
          "evidence": "水印存在只能说明某套工具留下了来源线索"
        },
        "boundary": {
          "answer": "水印和检测器都可能失效，技术检测不能替代法律判断与治理责任。",
          "evidence": "压缩、截图或新模型都可能使它失效"
        }
      }
    ]
  }
