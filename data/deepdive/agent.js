...
// 新版教学门禁补充：逐节说明 Agent 目标循环、状态机、组件、可靠性、自主边界和安全控制。
{
  const page = window.DEEPDIVE["agent"];
  const additions = [
    '<p>AI Agent 输入用户目标、当前状态、工具、权限、预算和停止条件，输出多步动作轨迹与经外部验证的任务结果。它比单轮聊天多了由系统主动选择下一动作并根据反馈继续的控制循环；模型负责提出决策，执行器才真正操作资源。自主不等于无限权限或保证完成。</p>',
    '<p>Agent 循环输入目标、已完成状态、工具观察、剩余预算和授权，输出下一候选动作、执行结果与更新状态。每轮先评估状态，再由策略和权限检查动作，执行后读取真实回执，决定继续、停止、失败或等待确认；循环记录的是状态动作观察，不要求公开私有思维链。没有外部观察就不能宣告完成。</p>',
    '<p>网页案例输入摘要目标、不可信网页、最多四轮、禁止读私密文件、发送授权和消息回执，输出安全摘要、被拒动作、待确认草稿或已验证发送。它解决不可信网页试图诱导 Agent 读取私密文件并外发的问题。网页指令不能扩大权限，发送必须暂停确认；确认后仍核对收件人内容与消息 ID。达到预算、重复动作、依赖失败或缺授权都要退出。</p>',
    '<p>Agent 架构输入目标和环境，输出由决策模型、工具执行器、规划状态、记忆检索与控制器组成的系统。模型理解与提出动作，工具访问世界，规划维护依赖，记忆保存可追溯状态，控制器强制权限预算停止；这些组件通过版本化接口连接。模型能力决定候选上限，但不能替代执行权限和真实状态。</p>',
    '<p>多步可靠性输入必要步骤数 m、简化单步成功率 s、步骤依赖、检查点、重试和回滚，输出理想化全链成功概率 Pchain 与真实任务指标。在独立同概率假设下 Pchain=s^m，s=0.9、m=10 得约 35%；真实步骤相关且有恢复，所以公式只是风险直觉。应缩短链路并逐步验证，而不是把单步分数直接当任务成功率。</p>',
    '<p>自主度选择输入路径可预知性、环境变化、错误成本、可验证性和运行预算，输出确定性工作流、受限 Agent 或混合系统。固定步骤写进工作流，真正未知的局部交给模型判断，并在检查点返回确定性控制；混合通常更可测可复现。越自主不代表越先进，可能只增加成本和失控面。</p>',
    '<p>安全控制输入工具能力、主体身份、资源、动作参数、不可信内容、可逆性和用户批准，输出允许、拒绝、沙箱模拟或人工确认。最小权限执行器在每次调用前重新鉴权，高危下单转账删除需明确确认并使用幂等与审计；提示注入文字不能成为授权来源。模型建议永远不是权限凭证。</p>'
  ];
  ...
  renderedAgain[4] += '<div class="dd-formula"><math display="block" aria-label="独立同概率步骤的全链成功率"><mi>Pchain</mi><mo>=</mo><msup><mi>s</mi><mi>m</mi></msup></math></div>';
