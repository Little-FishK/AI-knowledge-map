...
window.DEEPDIVE["agent"] = {
  ...
  thesis: "AI Agent 是一个能<b>自主循环</b>「思考 → 调工具 → 看结果 → 再思考」，直到完成任务的 LLM 系统。它把大模型从「聊天对象」变成「能做事的执行者」：给它一个目标，它自己拆解、多步执行、看着结果调整。工具调用是它的手，大模型是它的脑。",
  html: `
<div class="dd-goals">
...
<section class="dd-sec">
  ...
  <figure class="dd-fig">
    <svg viewBox="0 0 420 210" role="img" aria-label="Agent 的思考-行动-观察循环">
      <rect x="140" y="20" width="140" height="34" rx="8" fill="#21252d" stroke="#6b8cbe"/><text x="210" y="42" text-anchor="middle" class="svg-tn" font-size="12">① 思考 / 规划</text>
      ...
      <text x="70" y="110" text-anchor="middle" class="svg-t" font-size="11">据结果再想</text>
    ...
    <figcaption>图 1　Agent 循环：思考下一步该干嘛 → 行动（调一个工具）→ 观察结果 → 拿着结果再思考……如此往复，直到任务完成。「行动」这一步，就是工具调用（见其深读页）。</figcaption>
  </figure>
  <div class="dd-note intuition"><b>和思维链的区别</b>　思维链是「在脑子里」把推理写出来；Agent 循环是「真的动手」——每一步都可能去外部世界<b>查一下、改一下、看一下真实结果</b>，再据此调整。它把推理和行动交替起来（这也是 ReAct 的思路，见对应节点）。</div>
</section>

<section class="dd-sec">
  <h2><span class="dd-n">3</span>它由哪些零件搭成<span class="dd-badge eng">工程</span></h2>
...
<section class="dd-sec">
  <h2><span class="dd-n">5</span>自主，还是写死流程<span class="dd-badge intuition">直觉</span></h2>
...
<section class="dd-sec">
  <h2><span class="dd-n">6</span>安全：它能真的动手，风险更大<span class="dd-badge eng">安全</span></h2>
...
<section class="dd-sec">
  ...
  <ol class="dd-chain">
    ...
    <li>它靠「思考→行动→观察→再思考」的循环自主运转，行动即工具调用。<span>（§2）</span></li>
    <li>由大模型（脑）、工具调用（手）、规划、记忆搭成。<span>（§3）</span></li>
    <li>它比聊天难，因为错误随步数连乘衰减，还会打转、被幻觉连累、成本高。<span>（§4）</span></li>
    <li>所以不必盲目全自主：能写死的写死，关键处才用 Agent 判断（混合）。<span>（§5）</span></li>
    <li>它能真动手，提示注入劫持工具的破坏面更大，高危需人在回路。<span>（§6）</span></li>
  </ol>
