...
<p>所谓“按 P 取期望”，就是用事件真实出现的概率作权重求加权平均：<code>E<sub>P</sub>[f(x)]=ΣₓP(x)f(x)</code>。</p>
        ...
        <p>这一组约定描述了后续计算共同使用的<b>概率对象与阅读规则</b>，用于避免把真实频率 P、模型预测 Q 和已经发生的事件 x 混为一谈。输入是事件集合、P 与 Q，输出是可代入后续公式的概率和按 P 加权的期望。计算时先确认事件由 P 产生，再读取 Q 给该事件的概率，然后按 P 求长期平均。结果只能解释为给定分布下的理想平均量；现实中的 P 通常未知，样本频率也可能有抽样偏差。</p>`,
    },
    {
      title: '自信息：一次结果有多意外',
      kind: 'math',
      ...
      body: `
        ...
        <p>自信息指的是<b>某个实际结果发生后消除的不确定性</b>，解决“单次结果到底有多意外”的量化问题。输入是事件 x 及其真实概率 P(x)，输出是以 bit 表示的 I<sub>P</sub>(x)。它先对概率取以 2 为底的对数，再加负号；数值越大表示该结果在 P 下越罕见，但不能推出事件更重要、更有用或更正确。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>熵描述的是<b>一个真实分布在重复试验中的平均不确定性</b>，用于把不同事件的单次意外程度汇成一个分布级量。输入是完整分布 P，输出是平均 bit/次；计算先求每个事件的自信息，再按 P(x) 加权相加。熵更高表示长期平均更难预测，却不表示每个事件都罕见，也不能直接评价答案质量；编码解释还依赖长期、无损和分布稳定等前提。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>交叉熵可以理解为<b>数据按 P 出现、预测却由 Q 给出时的平均对数损失</b>，它解决如何用一个数比较模型概率与真实事件的问题。输入是 P 和 Q，输出是 bit/事件的 H(P,Q)；流程先让 P 决定事件频率，再读取 Q(x)，然后计算负对数并按 P 平均。数值较低表示 Q 给真实事件的平均概率较高，但不同数据分布、对数底数或预测单位下的数值不能直接横比。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>KL 散度描述<b>使用 Q 代替 P 后增加的平均编码代价</b>，用于分离“数据本身的不确定性”和“模型概率放错位置的额外损失”。输入是有方向的 P 与 Q，输出是 D<sub>KL</sub>(P∥Q)；计算先求交叉熵，再减去 P 自身的熵。零表示 Q 在 P 会发生的事件上完全匹配，较大表示额外代价更高；方向不可交换，且样本估计、零概率和未知 P 都限制实际解释。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>这项比较展示了<b>几何坐标差与概率编码代价是两种不同问题</b>，用于防止把 KL 当作普通距离。输入都是概率向量 P、Q；欧氏距离输出对称的几何长度，KL 输出有方向的平均对数代价。前者先逐坐标求差、平方并求和，再开方；后者按 P 加权概率比的对数。数值较小都可表示某种接近，但单位和含义不同，不能互换、相加或据此宣称一种度量永远更好。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>困惑度把<b>平均对数损失还原成等效分支数</b>，用于让抽象的 bit/token 更容易比较。输入是注明对数底数的平均 token 损失，输出是 PPL；先确认单位，再用对应指数函数还原。更低的 PPL 表示模型平均给真实 token 更高概率，但不代表候选数、事实性或推理能力；不同 tokenizer、语料和单位会改变数值，因此跨设置比较必须统一口径。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>这个完整手算展示<b>同一组 P、Q 怎样依次产生熵、交叉熵、KL 和困惑度</b>，用来核对五个量的角色是否连贯。输入是三种天气的 P 与 Q，输出是四个带单位的汇总量；计算先由 P 求熵，再用 P 加权 Q 的码长，然后相减得到 KL，最后指数化得到 PPL。结果说明 Q 低估雪造成主要额外代价，但这个三事件例子只验证算术与解释链，不能代表连续分布、有限样本估计或真实语言模型。</p>`,
    },
    {
      ...
      body: `
        ...
        <p>经验交叉熵描述<b>有限文本样本上的平均负对数概率</b>，用于在未知 P 时提供可计算的训练信号。输入是 token 序列及模型逐步概率，输出是序列或批次平均损失；先累加真实 token 的负对数概率，再除以有效 token 数并跨样本平均。训练值下降表示更贴合当前样本，验证值更适合估计新数据表现，但二者都有抽样和分布误差，也不能替代事实、安全与任务评测。</p>`,
    },
    {
      title: '常见误区与学习路线',
      role: 'reference',
