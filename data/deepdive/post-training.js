...
<p><b>文本似然</b>是模型给训练文本及其续写分配的相对可能性；预训练提高它，是为了学会数据中常见的语言规律，而不是判断“用户现在希望我怎么做”。后训练改用更接近产品目标的数据和反馈，塑造指令遵循、风格、工具使用、安全边界与推理策略。<b>后训练是阶段总称，微调是其中使用的参数更新手段。</b></p>
<p>后训练的输入是基座模型、指令示范、偏好比较或可验证反馈，输出是参数更新后的助手模型和相应评测记录。各阶段都通过“生成候选—比较目标—计算损失或奖励—更新参数”改变回答分布。结果应解释为目标行为的触发概率发生变化，不是模型突然拥有了权威事实；若基础能力、事实性或少数语言切片回归，就不能仅凭总偏好分数宣布成功。</p></section>
...
<p>每个训练样本输入一条指令和可选上下文，目标输出是示范回答中的 token 序列；模型像预训练一样逐 token 降低交叉熵。训练后，格式遵循率或拒答准确率提高，表示模型更常复现示范行为，并不表示未覆盖场景也会正确。示范之间若标准冲突、语言失衡或充满模板，模型会把这些问题一并学进去。</p>
...
<p>输入是提示 <code>x</code>、被偏好的回答 <code>y⁺</code>、较差回答 <code>y⁻</code>，以及作为行为锚点的参考模型。训练输出仍是更新后的策略模型：它应拉大好回答与差回答的相对分数，但又不能为迎合偏好数据而完全丢掉原有语言能力。</p>
<div class="dd-formula" data-formula-id="post-training-preference-objective" data-display="mathml"><math display="block" aria-label="总目标等于偏好收益减去 lambda 乘当前策略与参考策略的 KL 偏离"><mrow><mi>J</mi><mo>(</mo><mi>θ</mi><mo>)</mo><mo>=</mo><msub><mi>R</mi><mtext>偏好</mtext></msub><mo>(</mo><mi>θ</mi><mo>)</mo><mo>−</mo><mi>λ</mi><mo>·</mo><mi>KL</mi><mo>(</mo><msub><mi>π</mi><mi>θ</mi></msub><mo>∥</mo><msub><mi>π</mi><mtext>参考</mtext></msub><mo>)</mo></mrow></math></div>
<p class="dd-formula-note"><b>怎样读式子：</b><code>θ</code> 是正在更新的模型参数，<code>J</code> 是要提高的总目标，<code>R偏好</code> 衡量回答符合偏好数据的程度。<code>πθ</code> 是当前模型的回答分布，<code>π参考</code> 是更新前或 SFT 模型的分布；<code>KL</code> 衡量当前分布偏离参考分布多少，<code>λ</code> 控制“追求偏好”与“别偏离太远”的权衡。这里是帮助理解 RLHF 类目标的结构式；DPO 会把比较数据写成可直接优化的分类式，不必显式训练同一个奖励模型。</p>
<p>偏好胜率提高表示在当前标注者、提示和候选生成方式下更常选中目标回答，不是普遍质量或事实正确率。偏好有群体差异，参考模型和 λ 也会改变结果；必须另测迎合、冗长、事实性、拒答和跨群体表现。</p></section>
...
<p>可以用单元测试、答案检查器、证明器或过程验证器给反馈。输入是题目、模型生成的答案或步骤以及验证规则，输出是通过/失败、分数或具体出错步骤。结果奖励稀疏但定义清楚，过程反馈更早指出哪一步偏离，却需要昂贵标注且可能偏好特定解法。模型由此学习拆解、搜索、检查与修正，形成推理模型的重要训练来源。</p><p>验证分数上升只说明模型更会满足当前检查器。验证器不是事实本身：测试覆盖、证明器公理和规则代码决定可见目标。弱验证器会被奖励黑客利用，所以要保留隐藏测试、对抗样例和独立终验，并监控训练奖励与真实任务指标是否背离。</p></section>

<section class="dd-sec"><h2><span class="dd-n">5</span>它改变什么、不改变什么<span class="dd-badge intuition">边界</span></h2>
<p class="dd-lead">看到后训练指标改善时，怎样区分新增能力、能力被更稳定调用和表面风格变化？</p>
