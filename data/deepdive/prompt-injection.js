...
// 新版教学门禁补充：逐节说明注入来源、信任边界、权限不变量和防御验证。
{
  const page = window.DEEPDIVE["prompt-injection"];
  const additions = [
    '<p>提示注入识别输入用户目标、模型读到的外部内容和候选动作，输出哪些文字是不可信数据、哪些动作偏离授权意图。攻击者把命令写进网页邮件文档或工具返回，模型可能把它误当可服从指令；被劫持的是行为选择而非服务器权限本身。是否造成实际伤害取决于模型外执行器最终授予的权限。</p>',
    '<p>成因分析输入角色标记、自然语言上下文和模型外授权层，输出“哪些边界只是统计信号、哪些边界可强制执行”的判断。系统开发者用户工具角色会影响模型，但内容仍由同一模型解释；新颖对抗文本可能让它误分指令和数据。不能靠转义根治自然语言歧义，却能用独立权限和策略执行器阻断越权结果。</p>',
    '<p>注入分类输入恶意文字的控制者和进入上下文的渠道，输出直接注入或间接注入及对应受害者。直接注入由当前用户在对话中发起，间接注入由攻击者藏在网页邮件文档或工具结果中；后者常让无辜用户成为受害者。分类帮助定位入口，但两类都不能获得超出当前身份的工具权限。</p>',
    '<p>网页案例输入“总结 URL”的用户目标、不可信网页、模型提出的文件与发送动作以及当前授权，输出安全摘要、被拒动作和需要用户确认的草稿。规则是不可信内容可以作为摘要数据，却不能扩大权限、改变高层目标或批准副作用；逐步检查动作来源、必要性、主体资源权限和确认。结果拒绝读私密文件与外发，只保留摘要。</p>',
    '<p>工具风险分析输入 Agent 可调用的工具、凭证范围、动作可逆性和确认机制，输出注入成功后的最大影响面。只聊天的模型主要产生误导或上下文泄露，能读写付款的代理还可能造成真实副作用；工具能力和自主度会放大后果。模型提议始终只是候选动作，执行器必须最小授权、幂等并对高风险动作确认。</p>',
    '<p>纵深防御输入干净与带攻击文档、权限不变量、工具日志和正常任务样本，输出未授权调用率、秘密泄露率、任务成功率和误拒率。组合最小权限、内容来源标记、独立策略、人工确认和执行前检查，并用编码跨语言多轮攻击验证每层。通过标准是危险提议不能穿过授权边界，不是模型每次都口头识破攻击。</p>',
    '<p>概念区分输入攻击者目标、输入渠道、受害者和被突破的边界，输出提示注入、间接注入或越狱标签。提示注入侧重劫持系统行为，越狱侧重突破模型内容安全限制；手法可以重叠，同一输入也可能同时属于两者。标签用于选择测试与控制，不能因为称为越狱就忽略工具权限风险。</p>'
  ];
