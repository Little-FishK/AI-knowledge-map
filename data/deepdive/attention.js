...
<section class="dd-sec">
  ...
  <p>理解一个词，几乎总要看它和句子里<b>哪些别的词</b>有关。处理「她」，就得联系到「小红」；而「小红」可能在好几个词之外。难点有二：<b>相关的词可能离得很远</b>，而且<b>该看谁是随内容变的</b>——换句话说，不能靠固定规则，得让模型自己动态判断。注意力机制可以理解为让模型在处理每个词时自己决定该看句中哪些词的机制：它输入一串词的表示，先让每个词根据内容与别的词逐一比较，再输出每个词应重点看向谁的判断。它要解决的正是怎样不靠固定规则、为每个词找到该看的相关词这一问题；某个词被判断为重点看，表示它在当前内容下与该词更相关，而不表示它在句中离得最近。</p>
...
<section class="dd-sec">
  ...
  <p class="dd-lead">怎么让模型「自己决定该看哪些词」？答案是一套三步的匹配-加权流程。这套机制可以理解为把该看谁拆成查询、键、值三个角色的计算方式，用于解决模型该重点看句中哪些词的问题；它输入每个词的查询、键、值向量，输出该词汇总全场后的新表示。</p>
  <p>每个词先产出三个向量，各有分工：</p>
...
<section class="dd-sec">
  ...
  <p>第一把键更匹配，所以它的 Value 权重更大；输出却不是复制 <code>v₁</code>，而是两个 Value 的混合。Key 决定“取多少”，Value 决定“取什么”，二者不能混为一谈。这段手算描述缩放点积注意力如何把分数变成一次加权混合，用于解决权重到底怎样落到具体数值的问题；它输入查询、键、值向量，先做点积与缩放，再经 softmax 得到权重并加权求和，输出一个混合向量。权重更大表示这把键更匹配，但它不是解释概率。</p>
...
<section class="dd-sec">
  ...
  <p>最常见的一种叫<b>自注意力（self-attention）</b>：Q、K、V <b>全部来自同一句话</b>。也就是说，句子里的每个词都在看这句话里的所有词（包括自己），据此更新自己的表示。我们的运行示例——「她」在同一句里找到「小红」——就是自注意力。自注意力可以理解为查询、键、值都来自同一句话的注意力，用于解决句子内部各词如何互相参照的问题；它输入同一句话的词表示，先让每个词与同句所有词比较，再据权重更新，输出每个词看过全句后的新表示。某个词主要看向另一个词，表示两者在句内更相关，但这只是同一句内部的关系。</p>
...
<section class="dd-sec">
  ...
  <p>从输入输出看，它接收整段序列中所有 token 的表示，并同时输出每个位置融合可见上下文后的新表示；“并行”指这些位置不必像 RNN 那样等待前一个时间步完成。注意力之所以是转折点，可以理解为它用一层内的直接连接替代了 RNN 的逐步传递，用于解决长距离依赖难学和无法并行两个瓶颈；计算时每个位置先与所有可见位置直接匹配，再并行地一次算完。路径更短表示远距信息更容易学到，但更短的路径不保证权重一定落在正确位置。</p>
...
<section class="dd-sec">
  ...
  <p>所以用<b>多头注意力</b>：在同一层里并行运行多组投影，每组有自己的 Q/K/V 子空间，从不同角度形成信息混合。有些头会呈现语法、指代或位置模式，也有许多头相互冗余；“每头都有清晰固定职责”不是架构保证。多头注意力可以理解为在同一层并行跑多组注意力、各看一种相关方式的机制，用于解决单组注意力只能按一种方式衡量相关的问题；它输入同一批词表示，先在各子空间分别做注意力，再把多组结果拼接投影，输出综合多视角的新表示。某些头呈现语法或指代模式，表示分工可能在训练中形成，但这种分工是学出来的而非预先指定。</p>
...
<section class="dd-sec">
  ...
  <p>在<b>标准全局自注意力</b>中，设 <code>n</code> 是序列的 token 数；每个位置都要和所有位置算一次匹配，因此注意力分数矩阵有 <code>n²</code> 个元素。该部分的计算量与朴素显存占用随长度平方增长。实际总成本还包含投影与前馈层，FlashAttention、滑窗或稀疏注意力也会改变显存常数或复杂度。平方级开销可以理解为每个词都要与所有词两两匹配带来的成本，用于解决为什么长上下文又慢又贵这一问题；它输入序列全部位置的表示，先让每个位置与所有位置两两算分，再对整张分数矩阵做归一化与加权，输出每个位置的新表示。开销随长度平方增长表示长度翻倍成本翻四倍，但实际总成本还包含投影与前馈。</p>
