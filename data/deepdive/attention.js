...
<section class="dd-sec">
  ...
  <p>理解一个词，几乎总要看它和句子里<b>哪些别的词</b>有关。处理「她」，就得联系到「小红」；而「小红」可能在好几个词之外。真正的难题在于：<b>相关的词可能离得很远</b>，而且<b>该看谁是随内容变的</b>——不能靠固定规则（比如"只看前三个词"）来决定。注意力就是专门解决这个难题的一套机制：让模型自己决定每个位置该看向序列中的哪些位置，而不是由外部规则指定。</p>
    <p>它怎样做到？每个词先产出一组表示，再逐个和其他词比较内容相关度，最后按相关度高低汇总信息——相关度高的词会贡献更多。输入是序列里全部词的一组表示，输出是每个词看过全序列后的新表示。正因为权重来自内容比较，某个词被看重，就表示它在当下与当前词更相关，而不是因为它在句子里离得最近。</p>
  <div class="dd-note intuition"><b>旧办法为什么不够</b>　在注意力之前，主流是循环网络（RNN）：像读书一样<b>按顺序</b>一个词一个词处理，把“记忆”沿链条往后传。长路径会让早先信息更难保留，而且时间步之间难以并行；门控 RNN 能缓解但不能消除这两个结构约束。注意力的权重虽由内容驱动，但注意力本身不能保证模型一定把权重放到正确位置——它提供的是一种机制，而非正确性的保证。第 5 节会逐项比较。</div>
</section>
...
<section class="dd-sec">
  ...
  <p>查询、键、值都来自同一句话的注意力，称为<b>自注意力（self-attention）</b>。它解决的问题是：句子内部各词如何互相参照才能理解彼此的角色和关系——比如「她」需要在同一句中定位「小红」。</p>
  <p>自注意力输入同一句话的词表示，先让每个词与同句所有词比较相关度，再按权重汇总输出每个词看过全句后的新表示——我们的运行示例「她」在同一句里找到「小红」，就是这样。某个词主要看向另一个词，就表示两者在句内更相关。</p>
  <p>但这只是同一句内部的关系。换成跨句，就需要另一种变体：交叉注意力（见下）。</p>
