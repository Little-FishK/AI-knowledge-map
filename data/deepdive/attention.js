...
<section class="dd-sec">
  ...
  <ul class="dd-steps">
    <li><b>长距离依赖不再衰减</b>。RNN 里隔 n 个词要走 n 步，梯度连乘会指数级衰减（这正是<b>梯度消失</b>）；注意力让任意两位置的路径长度都是 <b>1</b>，多远都能直接对上。</li>
...
<section class="dd-sec">
  ...
  <p>既然每个位置都要和<b>所有</b>位置算一次匹配，那么算量和显存占用都是序列长度的<b>平方</b>。长度翻一倍，开销约变<b>四倍</b>。</p>
  ...
  <ul class="dd-steps">
    ...
    <li>它也是<b>「中间迷失」</b>的成因之一：位置越多，每个位置分到的注意力权重越薄，中段信息容易被稀释。</li>
  </ul>
...
<section class="dd-sec">
  ...
  <ol class="dd-chain">
    ...
    <li>代价是平方级开销，约束了上下文窗口，也是中间迷失的成因。<span>（§6）</span></li>
  </ol>
...
<section class="dd-sec">
  ...
  <div class="dd-table-wrap"><table class="dd-table">
    ...
    <tbody>
      ...
      <tr><td>注意力自己知道词序</td><td>不知道；单靠注意力打乱词序结果一样，词序要另外注入（见 Transformer 的位置编码）</td></tr>
    </tbody>
  </table></div>
</section>
...
<section class="dd-sec">
  ...
  <details class="dd-answers"><summary>参考答案</summary>
    <ol>
      ...
      <li>其一，任意两词路径长度为 1，长距离依赖不衰减；其二，所有位置并行计算。可并行使训练能吃满 GPU，从而「把模型和数据一起放大」在工程上可行，撑起了缩放定律。</li>
      <li>一组只能按一种方式衡量相关，多头让模型从多个视角同时看；不同头盯语法、指代、句首尾等，分工是训练中自发形成的。</li>
      <li>因为每个位置都要和所有位置算一次匹配，算量与显存都是长度的平方；它约束了上下文窗口的大小，也是中间迷失的成因之一。</li>
      <li>因为它只按内容匹配汇总、不含位置信息，打乱输入顺序算出的结果一样；词序要靠额外的位置编码注入。</li>
    </ol>
  </details>
</section>
