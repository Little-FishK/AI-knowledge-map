...
window.DEEPDIVE['context-engineering'].html = window.DEEPDIVE['context-engineering'].html
  .replace(
    '<div class="dd-formula">B<sub>window</sub> ≥ B<sub>rules</sub> + B<sub>task</sub> + B<sub>evidence</sub> + B<sub>history</sub> + B<sub>output reserve</sub></div>',
    '<div class="dd-formula" data-formula-id="context-engineering-budget"><math display="block" aria-label="窗口预算 B window 大于等于规则、任务、证据、历史和输出预留五项预算之和"><mrow><msub><mi>B</mi><mi>window</mi></msub><mo>≥</mo><msub><mi>B</mi><mi>rules</mi></msub><mo>+</mo><msub><mi>B</mi><mi>task</mi></msub><mo>+</mo><msub><mi>B</mi><mi>evidence</mi></msub><mo>+</mo><msub><mi>B</mi><mi>history</mi></msub><mo>+</mo><msub><mi>B</mi><mi>reserve</mi></msub></mrow></math></div><p class="dd-formula-note"><code>Bwindow</code> 是服务允许的总 token 窗口；<code>Brules</code>、<code>Btask</code>、<code>Bevidence</code>、<code>Bhistory</code> 分别是规则、当前任务、证据和历史的 token 占用；<code>Breserve</code> 是为模型输出和工具往返保留的预算。各项按同一 tokenizer 计数；不等式要求计划占用不能超过窗口，而不是鼓励把窗口全部填满。</p>'
  )
  .replace(
    '<div class="dd-formula">任务成功 = 必要证据召回 ∧ 装配后保留 ∧ 关键事实保真 ∧ 回答使用证据 ∧ 业务验收通过</div>',
    '<div class="dd-formula" data-formula-id="context-engineering-success-gates"><math display="block" aria-label="任务成功 S 等于召回 R、保留 K、事实保真 F、证据使用 U 和业务验收 A 五个布尔关卡的逻辑与"><mrow><mi>S</mi><mo>=</mo><mi>R</mi><mo>∧</mo><mi>K</mi><mo>∧</mo><mi>F</mi><mo>∧</mo><mi>U</mi><mo>∧</mo><mi>A</mi></mrow></math></div><p class="dd-formula-note"><code>S</code> 表示任务是否成功；<code>R</code> 表示必要证据被召回，<code>K</code> 表示装配后仍保留，<code>F</code> 表示压缩后关键事实保真，<code>U</code> 表示回答实际使用证据，<code>A</code> 表示最终通过业务验收。符号 ∧ 是逻辑“并且”：五项任一为假，S 就为假，不能靠其他环节高分补偿。</p>'
  )
  .replace('模型参数提供一般能力，而当前输出直接受这次调用中可见的上下文约束。</p>', '模型参数提供一般能力，而当前输出直接受这次调用中可见的上下文约束。</p><p>上下文工程的输入是本轮可用的规则、任务、证据、历史、工具结果与记忆，输出是带来源、顺序和预算的最终上下文包。装配器进行选择、过滤、压缩和排序；回答只说明模型基于最终包完成了生成，未进入包的信息本轮不可用。</p>')
  .replace('先按职责分层，才能处理冲突、更新和可观测性。</p>', '先按职责分层，才能处理冲突、更新和可观测性。</p><p>分层装配接收多来源内容，输出规则、任务、证据、记忆和示例五类带元数据区块。来源、时间、可信度和作用域决定冲突与更新方式；离模型更近只表示位置，不表示更可信。缺少来源标签时，系统无法可靠解释冲突或撤销错误记忆。</p>')
  .replace('窗口足够装下不等于模型能同等有效地使用全部内容，还要给输出和工具往返留余量。</p>', '窗口足够装下不等于模型能同等有效地使用全部内容，还要给输出和工具往返留余量。</p><p>预算器输入窗口上限、各类候选 token 和输出需求，输出受保护预算、可压缩预算与安全余量。先锁定规则、任务和输出，再按单位 token 价值选择证据与历史；结果表示装配计划可行，不保证模型能同等使用所有位置。</p>')
  .replace('召回只是找到候选，模型最终读到什么还取决于重排、切块、组织与引用。</p>', '召回只是找到候选，模型最终读到什么还取决于重排、切块、组织与引用。</p><p>证据装配输入召回候选、权限、时效和当前子问题，输出去重重排且保留邻接语境与引用的证据包。过滤、重排、扩展和结构化决定模型实际读到什么；召回命中只说明候选池含答案，不能证明最终上下文保留或回答正确使用。</p>')
  .replace('旧上下文会过期、相互矛盾并消耗注意力，因此记忆是一项有写入和淘汰规则的数据治理工作。</p>', '旧上下文会过期、相互矛盾并消耗注意力，因此记忆是一项有写入和淘汰规则的数据治理工作。</p><p>记忆治理输入历史事件、已验证事实、作用域和保留期，输出可更新、撤销和追溯的短期摘要或长期记录。只写跨任务稳定且有价值的事实，并保留未解决项和证据链接；流畅摘要不能解释为事实保真，敏感或过期记忆必须淘汰。</p>')
  .replace('检索文档和网页属于不可信数据，即使其中写着“忽略此前规则”。</p>', '检索文档和网页属于不可信数据，即使其中写着“忽略此前规则”。</p><p>信任隔离输入外部文本、来源与拟执行工具，输出标记为数据的内容和经过授权的动作。数据边界减少指令混淆，权限校验和人工批准阻止副作用；模型拒绝一次恶意句子不代表形成安全保证，编码、多语言和间接注入仍需测试。</p>')
  .replace('必须能重建“模型当时看见了什么”，否则失败只能被笼统归咎于模型。</p>', '必须能重建“模型当时看见了什么”，否则失败只能被笼统归咎于模型。</p><p>可观测性输入最终上下文、片段元数据、裁剪日志、版本和输出，输出可重放的失败轨迹与首次失效关卡。逐层比较召回、排序、保真、引用和答案，并用消融定位贡献；脱敏是边界，日志不能为了调试重新泄露用户秘密。</p>')
  .replace('容量解决的是能否放入，工程解决的是是否放对、摆对并及时更新。</p>', '容量解决的是能否放入，工程解决的是是否放对、摆对并及时更新。</p><p>长窗口评估输入候选材料、位置、长度、成本和任务结果，输出容量与有效利用的分离曲线。更多 token 会增加冲突、延迟和中间迷失概率；较短但证据充分的包可能更好。窗口变长只扩大可放入上限，不能替代检索、排序、更新和验证。</p>')
  .replace('最终答案变好或变差时，怎样区分是召回、筛选、压缩、排序还是模型本身造成的？</p>', '最终答案变好或变差时，怎样区分是召回、筛选、压缩、排序还是模型本身造成的？</p><p>管道评测输入冻结的问题集、知识快照、模型与装配变体，输出每个关卡、任务切片、成本和逐样本差异。一次只替换一个组件，并从必要证据召回一路检查到业务验收；任一布尔关卡失败都足以让任务失败，平均分不能补偿关键证据缺失。</p>');
