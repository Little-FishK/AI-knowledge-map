...
// 新版教学门禁补充：为六个核心章节补齐教学合同所需的局部证据。
{
  ...
  const additions = [
    '<p>RAG 可以理解为让模型回答前先检索相关资料、再照材料作答的方法，用于解决大模型会幻觉、知识有截止、够不到私有数据这三个硬伤；它输入用户问题和有权限访问的外部知识库，先把相关证据检索出来摆到模型面前，再让它照材料回答，输出带证据、可更新的回答。开卷作答通常表示回答更可核验，但结果更可核验不代表材料或回答自动正确，无可靠来源或任务不需要外部事实时不必强行检索。这里的似然指某段文字在上下文中有多可能出现；模型优化似然是为了续写更流畅，并不等于核验现实真相。</p>',
    '<p>检索增强生成流程可以理解为把检索、增强、生成三步串起来的处理链，用于解决怎样让模型基于外部材料而非记忆作答的问题；它输入问题、知识库片段及其版本和权限，先检索候选、再增强时保留证据边界，最后据材料生成带逐断言引用的答案，输出可核验的回答。引用只有真正支持对应断言才表示可信，但找不到证据时应明确无答案或追问，而不是补写不存在的事实。</p>',
    '<p>这一节描述检索增强到底补上了什么能力，用于解决它凭什么能缓解幻觉、还能更新和溯源的问题；它输入外部可更新的知识和当前问题，先把事实从模型权重移到运行时证据，再让模型据证据作答，输出更少无依据断言、可更新且可回查来源的答案。有出处通常表示答案可被检查，但可检查不等于出处可信或推理忠实，它只能缓解而不能消灭幻觉。</p>',
    '<p>这一节比较的是 RAG 与微调两条注入知识的路线，用于解决同样想让模型用我的知识该选哪个的问题；它输入要注入的是变化事实还是稳定行为，先判断知识是否常变、是否需溯源，再决定该走哪条路线，输出一个选型结论。知识常变又需溯源，通常表示更该走检索这条路，但检索不能替代行为训练，微调也不能可靠充当实时数据库。</p>',
    '<p>这一节描述检索增强最容易失效的边界，用于解决它为什么效果好坏多半不取决于模型的问题；它输入检索片段、上下文装配和最终回答，先看证据是否召回、是否被正确装配，再看生成是否忠实使用证据，输出对失败原因的定位。出现引用通常表示系统附了来源，但不证明来源支持断言，知识过期或权限错误也不能靠换更强模型修复。</p>',
    '<p>分层诊断可以理解为把失败按检索、装配、生成三层归因的方法，用于解决为什么端到端准确率下降还不足以指导修复的问题；它输入目标断言、必要证据、最终上下文和生成断言，先分别算证据覆盖、上下文精度、断言支持率，再据此定位失败层，输出可归因的修复方向。某项指标下降通常表示对应层出了问题，但指标下降只能定位生成忠实性，不能单独证明检索或业务任务合格。断言支持率 SupportRate 表示有证据支持的可核验断言数 Ssupported 占全部可核验断言数 Sverifiable 的比例。</p>'
  ];
