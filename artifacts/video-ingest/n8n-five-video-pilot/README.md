...
结构化判定见 `pilot.json`，批量复核汇总见 `shadow-batch-report.json`。每条视频自己的 `context.json`、`proposal.json`、`proposal.md`、`shadow/assessment.json` 和 `shadow/review.json` 位于同级的对应 slug 目录；原始音视频、转录、关键帧与 OCR 位于仓库外的本机 `video/raw/video/` 数据目录。
