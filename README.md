...
一个无需后端即可运行的 AI 学习知识网：用有方向、有类型的关系连接概念，并把概念原理、软件目录、专业资料库、使用教程和学习进度放在同一个静态网站里。
...
- 62 个软件条目、11 个软件门类
- 4 个使用教程页、22 条正式视频资源
- 9 类一级来源、85 个二级来源与 85 份平台档案、1829 条专业资料（其中官方技术资料 1391 条）
- 覆盖全部节点的 9 层官方推荐学习路径
...
源码目录由一个本地 HTTP 服务提供即可（任意静态服务器，例如 `python -m http.server`）。它不等同于完整发布版：不加载英文 JSON，内容工作流提示也可能与正式站不同。它还依赖 `assets/`、`data/` 等本地资源，不能只下载一个 HTML 文件运行。直接双击 `index.html` 会以 `file://` 打开，样式与静态内容仍在，但地址栏路由依赖 `pushState`，只有 HTTP(S) 源下才能更新地址。

访客学习进度保存在浏览器本地存储中；清除浏览器数据会清除本地进度。网站也提供账号和跨设备同步：登录后通过 Supabase Auth 与数据库同步学习进度。登录、云同步和账号删除需要网络，本地浏览不等于这些在线功能可用。不同浏览器与不同来源（本地服务、正式站点）的存储相互独立。
...
页面使用无片段的路径地址：三个交互视图各有一个真实目录——地图在站点根目录 `/`，软件目录在 `/software/`，专业资料库在 `/library/`。条目详情和节点定位用查询参数表达，例如 `/library/?item=<id>`、`/?node=<id>`、`/software/?tutorial=<id>`；语言用 `?lang=` 携带并在导航时保留。普通地图节点详情仍属临时探索状态，不改变网址；复制完整地址即可分享或收藏真正的内容页，刷新以及浏览器前进、后退也会恢复对应视图。旧的 `#/...` 地址继续可用，加载时会就地升级为上面的路径形式。

地图支持 `/?node=supervised-learning` 一类节点定位地址，供独立内容页链接回地图；从这种地址进入后，选择其他节点会同步更新定位参数，关闭节点详情则回到站点根地址。语言与节点定位可以组合，例如 `/?lang=en&node=supervised-learning` 直接进入英文地图。正式网站同时提供 `/zh/concepts/<id>/` 和 `/en/concepts/<id>/` 独立内容页，导航根据所选语言进入对应页面。

路径地址需要真实文件，因此 `/library/` 与 `/software/` 由发布构建生成独立目录。地址栏更新依赖 `history.pushState`，浏览器只在 HTTP(S) 源下允许调用它，所以上述地址需要由一个本地或线上服务器提供；直接以 `file://` 打开时地图与内容仍会渲染，但地址栏不会随视图变化。

## 构建与部署

源码运行与发布构建是两种不同的使用方式。源码目录由本地 HTTP 服务提供；开发检查需要 Node.js 22 和项目依赖；发布还需生成独立内容页、搜索和清单等产物：
...
前端继续使用按顺序加载的普通脚本，不依赖打包器。`assets/router.js` 只用无片段的路径 + 查询参数地址；`assets/app.js` 只负责模块组装、顶级视图切换和 URL 路由应用；各功能位于 `assets/app/`：
