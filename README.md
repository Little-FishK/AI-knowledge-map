...
> 当前阶段：**网站成熟化 M1 进行中**。[GitHub Pages 旧站](https://little-fishk.github.io/AI-knowledge-map/)可以访问，但尚未同步当前本地功能。当前实施进度见 [docs/WEBSITE_MATURITY_PROGRESS.md](docs/WEBSITE_MATURITY_PROGRESS.md)；[docs/STATUS.md](docs/STATUS.md)为历史快照。内容覆盖不等于教学质量认证。
...
当前本地版本还支持 `#/map/supervised-learning` 一类节点定位地址，供独立内容页链接回地图；从这种地址进入后，选择其他节点会同步更新定位地址，关闭节点详情则返回 `#/map`。`?lang=en#/map/supervised-learning` 可直接进入英文地图，手动切换语言后地址中的语言参数也会更新。旧站尚未部署这些能力。Hash 定位不代替后续可索引的独立 HTML 页面。
...
多语言改造的语言合同、翻译边界和源内容基线见 [docs/I18N.md](docs/I18N.md)，统一中英名称及审核流程见 [docs/TERMINOLOGY.md](docs/TERMINOLOGY.md)。当前英文图谱大区、关系类型、全部 130 个节点及官方学习顺序的九个阶段均已发布。软件、教程、资料库和理解原理页等尚未翻译的内容类型仍按完整记录回退为中文。术语清单已覆盖 130/130 个节点，其中 130 条项目已批准、0 条待审。
