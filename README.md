...
v0.1 可以把同一份视频证据转成教程与概念两条独立提案。v0.2 在此基础上加入独立批准文件、dry-run 变更计划、确定性教程写入、概念补充队列、目标哈希、应用后门禁、失败自动恢复与显式回滚。v0.3 新增模型中立的只读影子复核、盲审任务包、结果导入和批量试跑。v0.4 可为双阶段一致的新节点生成包含节点、关系、学习路径、理解原理页和无重叠布局的完整预览包，并复用正式 L1–L3 门禁；当前正式图谱写入恒为 0。
